...
function updateenv() {
    ...
    REQUIREMENTS_PLOT=""
    REQUIREMENTS_TRADERAI=""
    REQUIREMENTS_TRADERAI_RL=""
    ...
    then
        ...
        read -p "Do you want to install dependencies for traderai [y/N]? "
        if [[ $REPLY =~ ^[Yy]$ ]]
        then
            REQUIREMENTS_TRADERAI="-r requirements-traderai.txt --use-pep517"
            read -p "Do you also want dependencies for traderai-rl or PyTorch (~700mb additional space required) [y/N]? "
            if [[ $REPLY =~ ^[Yy]$ ]]
            then
                REQUIREMENTS_TRADERAI="-r requirements-traderai-rl.txt"
            fi
        fi
    fi
    install_talib

    ${PYTHON} -m pip install --upgrade -r ${REQUIREMENTS} ${REQUIREMENTS_HYPEROPT} ${REQUIREMENTS_PLOT} ${REQUIREMENTS_TRADERAI} ${REQUIREMENTS_TRADERAI_RL}
    if [ $? -ne 0 ]; then
    ...
    if [ $? -ne 0 ]; then
        echo "Failed installing Traderpilot"
        exit 1
    fi

    echo "Installing traderUI"
...
function config() {
    echo_block "Please use 'traderpilot new-config -c user_data/config.json' to generate a new configuration file."
}
...
function install() {
    ...
    echo "You can now use the bot by executing 'source .venv/bin/activate; traderpilot <subcommand>'."
    echo "You can see the list of available bot sub-commands by executing 'source .venv/bin/activate; traderpilot --help'."
    echo "You verify that traderpilot is installed successfully by running 'source .venv/bin/activate; traderpilot --version'."
}
...
function help() {
    echo "usage:"
    echo "	-i,--install    Install traderpilot from scratch"
