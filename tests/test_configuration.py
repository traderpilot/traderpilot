...
from traderpilot.configuration.config_validation import validate_config_schema
from traderpilot.configuration.deprecated_settings import (
    check_conflicting_settings,
    process_deprecated_setting,
    process_removed_setting,
    process_temporary_deprecated_settings,
)
from traderpilot.configuration.environment_vars import _flat_vars_to_nested_dict
from traderpilot.configuration.load_config import (
    load_config_file,
    load_file,
    load_from_files,
...
def test_load_config_file(default_conf, mocker, caplog) -> None:
    ...
    file_mock = mocker.patch(
        "traderpilot.configuration.load_config.Path.open",
...
def test_load_config_file_error(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch(
        "traderpilot.configuration.load_config.Path.open", mocker.mock_open(read_data=filedata)
    )
...
def test_load_config_combine_dicts(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch("traderpilot.configuration.load_config.load_config_file", configsmock)
...
def test_from_config(default_conf, mocker, caplog) -> None:
    ...
    config_files = [conf1, conf2]
    mocker.patch("traderpilot.configuration.configuration.create_datadir", lambda c, x: x)
...
def test_print_config(default_conf, mocker, caplog) -> None:
    ...
    configsmock = MagicMock(side_effect=config_files)
    mocker.patch("traderpilot.configuration.configuration.create_datadir", lambda c, x: x)
    mocker.patch("traderpilot.configuration.configuration.load_from_files", configsmock)
...
def test_load_config_file_exception(mocker) -> None:
    mocker.patch(
        "traderpilot.configuration.configuration.Path.open",
...
def test_setup_configuration_with_arguments(mocker, default_conf, caplog, tmp_path) -> None:
    ...
    mocker.patch("traderpilot.configuration.configuration.create_datadir", lambda c, x: x)
    mocker.patch(
        "traderpilot.configuration.configuration.create_userdata_dir",
    ...
    assert log_has(f"Using user-data directory: {tmp_path / 'traderpilot'} ...", caplog)
    assert "user_data_dir" in config
...
def test_cli_verbose_with_params(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch("traderpilot.loggers.set_loggers", MagicMock)
    arglist = ["trade", "-vvv"]
    args = Arguments(arglist).get_parsed_arg()
...
def test__validate_traderai_include_timeframes(default_conf, caplog) -> None:
    conf = deepcopy(default_conf)
    conf.update(
        {
            "traderai": {
    ...
    conf["traderai"]["feature_parameters"]["include_timeframes"] = ["5m", "15m"]
    validate_config_consistency(conf)
    assert conf["traderai"]["feature_parameters"]["include_timeframes"] == ["1m", "5m", "15m"]

    conf.update({"analyze_per_epoch": True})
    with pytest.raises(
        OperationalException,
        match=r"Using analyze-per-epoch .* not supported with a TraderAI strategy.",
    ):
        validate_config_consistency(conf)
...
def test_pairlist_resolving_fallback(mocker, tmp_path):
    ...
    mocker.patch(
        "traderpilot.configuration.configuration.load_file",
...
def test_flat_vars_to_nested_dict(caplog):
    test_args = {
        "TRADEPILOT__EXCHANGE__SOME_SETTING": "true",
        "TRADEPILOT__EXCHANGE__SOME_FALSE_SETTING": "false",
        "TRADEPILOT__EXCHANGE__CONFIG__whatever": "sometime",
        "TRADEPILOT__EXIT_PRICING__PRICE_SIDE": "bid",
        "TRADEPILOT__EXIT_PRICING__cccc": "500",
        "TRADEPILOT__STAKE_AMOUNT": "200.05",
        "TRADEPILOT__TELEGRAM__CHAT_ID": "2151",
        "NOT_RELEVANT": "200.0",  # Will be ignored
        "TRADEPILOT__ARRAY": '[{"name":"default","host":"xxx"}]',
        "TRADEPILOT__EXCHANGE__PAIR_WHITELIST": '["BTC/USDT", "ETH/USDT"]',
        # Fails due to trailing comma
        "TRADEPILOT__ARRAY_TRAIL_COMMA": '[{"name":"default","host":"xxx",}]',
        # Object fails
        "TRADEPILOT__OBJECT": '{"name":"default","host":"xxx"}',
    }
    expected = {
        "stake_amount": 200.05,
        "exit_pricing": {
    ...
    assert log_has("Loading variable 'TRADEPILOT__EXCHANGE__SOME_SETTING'", caplog)
    assert not log_has("Loading variable 'NOT_RELEVANT'", caplog)


def test_setup_hyperopt_traderai(mocker, default_conf) -> None:
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch("traderpilot.configuration.configuration.create_datadir", lambda c, x: x)
    mocker.patch(
        "traderpilot.configuration.configuration.create_userdata_dir",
...
def test_setup_traderai_backtesting(mocker, default_conf) -> None:
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch("traderpilot.configuration.configuration.create_datadir", lambda c, x: x)
    mocker.patch(
        "traderpilot.configuration.configuration.create_userdata_dir",
    ...
    with pytest.raises(
        OperationalException, match=r".*--traderai-backtest-live-models parameter is only.*"
    ):
        validate_config_consistency(config)

    conf = deepcopy(config)
    conf["traderai"] = {"enabled": True}
    with pytest.raises(
    ...
    conf["timerange"] = None
    conf["traderai_backtest_live_models"] = False

    with pytest.raises(
        OperationalException, match=r".* pass --timerange if you intend to use TraderAI .*"
    ):
        validate_config_consistency(conf)
