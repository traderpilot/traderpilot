...
from traderpilot.data.btanalysis import BT_DATA_COLUMNS, evaluate_result_multi
from traderpilot.data.converter import clean_ohlcv_dataframe, ohlcv_fill_up_missing_data
...
def test_start(mocker, fee, default_conf, caplog) -> None:
    ...
    patch_exchange(mocker)
    mocker.patch("traderpilot.optimize.backtesting.Backtesting.start", start_mock)
    patched_configuration_load_config_file(mocker, default_conf)
    ...
    assert log_has("Starting traderpilot in Backtesting mode", caplog)
...
def test_backtesting_start(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch("traderpilot.data.history.get_timerange", get_timerange)
    patch_exchange(mocker)
    mocker.patch("traderpilot.optimize.backtesting.Backtesting.backtest")
    mocker.patch("traderpilot.optimize.backtesting.generate_backtest_stats")
    mocker.patch("traderpilot.optimize.backtesting.show_backtest_results")
    sbs = mocker.patch("traderpilot.optimize.backtesting.store_backtest_results")
    mocker.patch(
...
def test_backtesting_start_no_data(default_conf, mocker, caplog, testdatadir) -> None:
    ...
    mocker.patch(
        "traderpilot.data.history.history_utils.load_pair_history",
...
def test_backtesting_pairlist_list(default_conf, mocker, tickers) -> None:
    ...
    mocker.patch("traderpilot.plugins.pairlistmanager.PairListManager.refresh_pairlist")
...
def test_backtest__enter_trade_futures(default_conf_usdt, fee, mocker) -> None:
    ...
    mocker.patch(f"{EXMS}.get_max_pair_stake_amount", return_value=float("inf"))
    mocker.patch(
        "traderpilot.persistence.trade_model.price_to_precision", lambda p, *args, **kwargs: p
    )
    mocker.patch(f"{EXMS}.get_max_leverage", return_value=100)
    mocker.patch("traderpilot.optimize.backtesting.price_to_precision", lambda p, *args: p)
    patch_exchange(mocker)
    ...
    backtesting._set_strategy(backtesting.strategylist[0])
    mocker.patch("traderpilot.optimize.backtesting.Backtesting._run_funding_fees")
    pair = "ETH/USDT:USDT"
    row = [
        pd.Timestamp(year=2020, month=1, day=1, hour=5, minute=0),
        0.1,  # Open
    ...
    # traderpilot_liquidation_price = liq + (abs(open_rate - liq) * liq_buffer * side_1)
    ...
    mocker.patch(
        "traderpilot.wallets.Wallets.get_trade_stake_amount", side_effect=DependencyException
    )
...
def test_backtest_start_multi_strat(default_conf, mocker, caplog, testdatadir):
    ...
    mocker.patch("traderpilot.optimize.backtesting.Backtesting.backtest", backtestmock)
    ...
    mocker.patch.multiple(
        "traderpilot.optimize.optimize_reports.bt_output",
        text_table_bt_results=text_table_mock,
        text_table_strategy=strattable_mock,
    )
    mocker.patch.multiple(
        "traderpilot.optimize.optimize_reports.optimize_reports",
...
def test_backtest_start_multi_strat_caching(
    ...
    mocker.patch("traderpilot.optimize.backtesting.Backtesting.backtest", backtestmock)
    mocker.patch("traderpilot.optimize.backtesting.show_backtest_results", MagicMock())
