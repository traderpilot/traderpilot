...
def test_update_order_from_ccxt(caplog, time_machine):
    ...
    assert o.tp_pair == "ADA/USDT"
    assert o.tp_order_side == "buy"
    ...
    # https://github.com/traderpilot/traderpilot/issues/5404
...
def test_select_order(fee, is_short):
    ...
    trades[4].orders[1].tp_order_side = trades[4].exit_side
    order = trades[4].select_order(trades[4].exit_side, True)
    assert order is not None

    trades[4].orders[1].tp_order_side = "stoploss"
    order = trades[4].select_order("stoploss", None)
    assert order is not None
    assert order.tp_order_side == "stoploss"
...
def test_recalc_trade_from_orders_kucoin():
    # Taken from https://github.com/traderpilot/traderpilot/issues/9346
...
def test_select_filled_orders(fee):
    ...
    assert order.tp_order_side == "buy"
...
def test_order_to_ccxt(limit_buy_order_open, limit_sell_order_usdt_open):
    ...
    order1.tp_order_side = "stoploss"
    order1.stop_price = order1.price * 0.9
    order1.tp_trade_id = 1
