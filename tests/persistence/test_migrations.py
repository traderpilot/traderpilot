...
from traderpilot.persistence.base import ModelBase
...
def test_init_custom_db_url(default_conf, tmp_path):
    ...
    filename = tmp_path / "traderpilot2_test.sqlite"
...
def test_init_prod_db(default_conf, mocker):
    ...
    create_engine_mock = mocker.patch("traderpilot.persistence.models.create_engine", MagicMock())
...
def test_init_dryrun_db(default_conf, tmpdir):
    filename = f"{tmpdir}/traderpilot2_prod.sqlite"
...
def test_migrate(mocker, default_conf, fee, caplog):
    ...
    create_table_order = """CREATE TABLE orders (
                                id INTEGER NOT NULL,
                                tp_trade_id INTEGER,
                                tp_order_side VARCHAR(25) NOT NULL,
                                tp_pair VARCHAR(25) NOT NULL,
                                tp_is_open BOOLEAN NOT NULL,
    ...
    mocker.patch("traderpilot.persistence.models.create_engine", lambda *args, **kwargs: engine)
    ...
    assert log_has("Running database migration for trades - backup: trades_bak2, orders_bak0", caplog)
    ...
    assert orders[0].tp_order_side == "buy"
