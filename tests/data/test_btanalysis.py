...
from traderpilot.data.history import load_data, load_pair_history
from traderpilot.data.metrics import (
    calculate_cagr,
    calculate_calmar,
    calculate_csum,
...
def test_get_latest_backtest_filename(testdatadir, mocker):
    ...
    mocker.patch("traderpilot.data.btanalysis.json_load", return_value={})
...
def test_load_backtest_data_old_format(testdatadir, mocker):
    ...
    mocker.patch("traderpilot.data.btanalysis.load_backtest_stats", return_value=[])
...
def test_load_trades_from_db(default_conf, fee, is_short, mocker):
    ...
    init_mock = mocker.patch("traderpilot.data.btanalysis.init_db", MagicMock())
...
def test_load_trades(default_conf, mocker):
    db_mock = mocker.patch("traderpilot.data.btanalysis.load_trades_from_db", MagicMock())
    bt_mock = mocker.patch("traderpilot.data.btanalysis.load_backtest_data", MagicMock())
...
def test_calculate_max_drawdown_abs(profits, relative, highd, lowdays, result, result_rel):
    """
    Test case from issue https://github.com/traderpilot/traderpilot/issues/6655
