...
from traderpilot.data.history.history_utils import (
...
def test_load_data_7min_timeframe(caplog, testdatadir) -> None:
    ...
    assert log_has(
        ...
        "Use `traderpilot download-data` to download the data",
        caplog,
    )
...
def test_load_data_startup_candles(mocker, testdatadir) -> None:
    ltfmock = mocker.patch(
        "traderpilot.data.history.datahandlers.featherdatahandler.FeatherDataHandler._ohlcv_load",
...
@pytest.mark.parametrize(
    ...
    [
        ("ETH/BTC", "5m", "traderpilot/hello/world/ETH_BTC-5m.json", ""),
        ("ETH/USDT", "1M", "traderpilot/hello/world/ETH_USDT-1Mo.json", ""),
        ("Fabric Token/ETH", "5m", "traderpilot/hello/world/Fabric_Token_ETH-5m.json", ""),
        ("ETHH20", "5m", "traderpilot/hello/world/ETHH20-5m.json", ""),
        (".XBTBON2H", "5m", "traderpilot/hello/world/_XBTBON2H-5m.json", ""),
        ("ETHUSD.d", "5m", "traderpilot/hello/world/ETHUSD_d-5m.json", ""),
        ("ACC_OLD/BTC", "5m", "traderpilot/hello/world/ACC_OLD_BTC-5m.json", ""),
        ("ETH/BTC", "5m", "traderpilot/hello/world/futures/ETH_BTC-5m-mark.json", "mark"),
        ("ACC_OLD/BTC", "5m", "traderpilot/hello/world/futures/ACC_OLD_BTC-5m-index.json", "index"),
    ],
)
def test_json_pair_data_filename(pair, timeframe, expected_result, candle_type):
    fn = JsonDataHandler._pair_data_filename(
        Path("traderpilot/hello/world"), pair, timeframe, CandleType.from_string(candle_type)
    )
    assert isinstance(fn, Path)
    assert fn == Path(expected_result)
    fn = JsonGzDataHandler._pair_data_filename(
        Path("traderpilot/hello/world"),
        pair,
        timeframe,
...
@pytest.mark.parametrize(
    ...
    [
        ("ETH/BTC", "", "traderpilot/hello/world/ETH_BTC-trades.json"),
        ("ETH/USDT:USDT", "futures", "traderpilot/hello/world/futures/ETH_USDT_USDT-trades.json"),
        ("Fabric Token/ETH", "", "traderpilot/hello/world/Fabric_Token_ETH-trades.json"),
        ("ETHH20", "", "traderpilot/hello/world/ETHH20-trades.json"),
        (".XBTBON2H", "", "traderpilot/hello/world/_XBTBON2H-trades.json"),
        ("ETHUSD.d", "", "traderpilot/hello/world/ETHUSD_d-trades.json"),
        ("ACC_OLD_BTC", "", "traderpilot/hello/world/ACC_OLD_BTC-trades.json"),
    ],
)
def test_json_pair_trades_filename(pair, trading_mode, expected_result):
    fn = JsonDataHandler._pair_trades_filename(Path("traderpilot/hello/world"), pair, trading_mode)
    assert isinstance(fn, Path)
    assert fn == Path(expected_result)
...
def test_download_pair_history2(mocker, default_conf, testdatadir, ohlcv_history) -> None:
    json_dump_mock = mocker.patch(
        "traderpilot.data.history.datahandlers.featherdatahandler.FeatherDataHandler.ohlcv_store",
        return_value=None,
    )
    exchange = get_patched_exchange(mocker, default_conf)
    mocker.patch.object(exchange, "get_historic_ohlcv", return_value=ohlcv_history)
...
def test_refresh_backtest_ohlcv_data(
    ...
    dl_mock = mocker.patch("traderpilot.data.history.history_utils._download_pair_history")
...
def test_download_data_no_markets(mocker, default_conf, caplog, testdatadir):
    dl_mock = mocker.patch(
        "traderpilot.data.history.history_utils._download_pair_history", MagicMock()
    )
...
def test_refresh_backtest_trades_data(mocker, default_conf, markets, caplog, testdatadir):
    dl_mock = mocker.patch(
        "traderpilot.data.history.history_utils._download_trades_history", MagicMock()
    )
    mocker.patch(f"{EXMS}.markets", PropertyMock(return_value=markets))
    mocker.patch.object(Path, "exists", MagicMock(return_value=True))
    mocker.patch.object(Path, "unlink", MagicMock())
