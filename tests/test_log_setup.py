...
from traderpilot.loggers import (
    FTBufferingHandler,
    FtRichHandler,
    set_loggers,
    setup_logging,
...
from traderpilot.loggers.set_log_levels import (
...
def test_set_loggers_Filehandler(tmp_path):
    ...
    orig_handlers = logger.handlers
    logger.handlers = []
    logfile = tmp_path / "logs/tp_logfile.log"
    config = {
        "verbosity": 2,
        "logfile": str(logfile),
    }
...
def test_reduce_verbosity():
    setup_logging_pre()
    reduce_verbosity_for_bias_tester()
    prior_level = logging.getLogger("traderpilot").getEffectiveLevel()

    assert logging.getLogger("traderpilot.resolvers").getEffectiveLevel() == logging.WARNING
    assert logging.getLogger("traderpilot.strategy.hyper").getEffectiveLevel() == logging.WARNING
    # base level wasn't changed
    assert logging.getLogger("traderpilot").getEffectiveLevel() == prior_level

    restore_verbosity_for_bias_tester()

    assert logging.getLogger("traderpilot.resolvers").getEffectiveLevel() == prior_level
    assert logging.getLogger("traderpilot.strategy.hyper").getEffectiveLevel() == prior_level
    assert logging.getLogger("traderpilot").getEffectiveLevel() == prior_level
    # base level wasn't changed
