    ...
    default_conf.update(
        {
            ...
            "strategy_path": str(Path(__file__).parents[2] / "traderpilot/templates"),
        }
    )
...
def test_load_strategy_base64(dataframe_1m, caplog, default_conf):
    filepath = Path(__file__).parents[2] / "traderpilot/templates/sample_strategy.py"
...
def test_strategy_tp_load_params_from_file(mocker, default_conf):
    default_conf.update({"strategy": "StrategyTestV2"})
    del default_conf["max_open_trades"]
    mocker.patch(
