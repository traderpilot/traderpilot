...
from traderpilot.enums import ExitCheckTuple, ExitType, HyperoptState, SignalDirection
from traderpilot.exceptions import OperationalException, StrategyError
from traderpilot.optimize.hyperopt_tools import HyperoptStateContainer
...
from traderpilot.strategy.hyper import detect_parameters
...
def test_traderai_not_initialized(default_conf) -> None:
    strategy = StrategyResolver.load_strategy(default_conf)
    strategy.tp_bot_start()
    with pytest.raises(OperationalException, match=r"traderAI is not enabled\."):
        strategy.traderai.start()


def test_advise_all_indicators_copy(mocker, default_conf, testdatadir) -> None:
    strategy = StrategyResolver.load_strategy(default_conf)
    aimock = mocker.patch("traderpilot.strategy.interface.IStrategy.advise_indicators")
    timerange = TimeRange.parse_timerange("1510694220-1510700340")
    data = load_data(testdatadir, "1m", ["UNITTEST/BTC"], timerange=timerange, fill_up_missing=True)
...
def test_tp_stoploss_reached(
    ...
    sl_flag = strategy.tp_stoploss_reached(
...
def test_should_sell(default_conf, fee) -> None:
    ...
    strategy.min_roi_reached = MagicMock(return_value=True)
    strategy.tp_stoploss_reached = MagicMock(
...
def test_auto_hyperopt_interface_loadparams(default_conf, mocker, caplog):
    ...
    mocker.patch(
        "traderpilot.strategy.hyper.HyperoptTools.load_params", MagicMock(side_effect=ValueError())
    )
