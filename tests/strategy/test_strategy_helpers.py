...
def test_informative_decorator(mocker, default_conf_usdt, trading_mode):
    ...
    assert len(strategy._tp_informative) == 7  # Equal to number of decorators used
    informative_pairs = [
    ...
    mocker.patch(
        "traderpilot.data.dataprovider.DataProvider.historic_ohlcv", side_effect=test_historic_ohlcv
    )
