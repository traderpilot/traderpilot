...
def test_add_stoploss_on_exchange(mocker, default_conf_usdt, limit_order, is_short, fee) -> None:
    ...
    traderpilot.exit_positions(trades)
...
def test_handle_stoploss_on_exchange(
    ...
    assert traderpilot.handle_stoploss_on_exchange(trade) is False
    assert stoploss.call_count == 1
    ...
    assert traderpilot.handle_stoploss_on_exchange(trade) is True
    ...
    trade.is_open = True
    traderpilot.handle_stoploss_on_exchange(trade)
...
def test_handle_stoploss_on_exchange_emergency(
    ...
    trade.orders.append(
        Order(
            tp_order_side="stoploss",
            tp_pair=trade.pair,
            tp_is_open=True,
            tp_amount=trade.amount,
            tp_price=trade.stop_loss,
            order_id="107",
            status="open",
        )
    )
    traderpilot.config["trailing_stop"] = True
...
def test_handle_stoploss_on_exchange_partial_cancel_here(
    ...
    # Traderpilot cancels the stop - but cancel returns a partial filled order.
    stoploss_order_hit = MagicMock(
        return_value={
            "id": "101",
            "status": "open",
...
def test_create_stoploss_order_invalid_order(
    ...
    caplog.clear()
    rpc_mock.reset_mock()
    traderpilot.create_stoploss_order(trade, 200)
    assert trade.has_open_sl_orders is False
...
def test_handle_stoploss_on_exchange_trailing(
    ...
    # setting stoploss
    traderpilot.strategy.stoploss = 0.05 if is_short else -0.05

    # setting stoploss_on_exchange_interval to 60 seconds
    traderpilot.strategy.order_types["stoploss_on_exchange_interval"] = 60
    ...
    stoploss_order_mock.assert_called_once_with(
        amount=30,
        pair="ETH/USDT",
        order_types=traderpilot.strategy.order_types,
...
def test_handle_stoploss_on_exchange_trailing_error(
    ...
    traderpilot.handle_trailing_stoploss_on_exchange(trade, stoploss_order_hanging)
...
def test_stoploss_on_exchange_price_rounding(
    ...
    traderpilot.handle_trailing_stoploss_on_exchange(open_trade_usdt, {})
...
def test_handle_stoploss_on_exchange_custom_stop(
    ...
    # setting stoploss
    traderpilot.strategy.custom_stoploss = lambda *args, **kwargs: -0.04
    ...
    traderpilot.strategy.order_types["stoploss_on_exchange_interval"] = 0
    cancel_order_mock.assert_not_called()
    stoploss_order_mock.assert_not_called()
...
def test_tsl_on_exchange_compatible_with_edge(mocker, edge_conf, fee, limit_order) -> None:
    ...
    # setting stoploss
    traderpilot.strategy.stoploss = -0.02
    ...
    traderpilot.active_pair_whitelist = traderpilot.edge.adjust(traderpilot.active_pair_whitelist)
...
def test_execute_trade_exit_sloe_cancel_exception(
    ...
    traderpilot = get_patched_traderpilotbot(mocker, default_conf_usdt)
    mocker.patch(f"{EXMS}.cancel_stoploss_order", side_effect=InvalidOrderException())
    mocker.patch("traderpilot.wallets.Wallets.get_free", MagicMock(return_value=300))
    create_order_mock = MagicMock(
        side_effect=[
            {"id": "12345554"},
            {"id": "12345555"},
...
def test_may_execute_trade_exit_after_stoploss_on_exchange_hit(
    ...
    traderpilot = TraderpilotBot(default_conf_usdt)
    traderpilot.strategy.order_types["stoploss_on_exchange"] = True
    patch_get_signal(traderpilot, enter_long=not is_short, enter_short=is_short)
