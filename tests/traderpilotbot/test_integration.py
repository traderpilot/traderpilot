...
def test_may_execute_exit_stoploss_on_exchange_multi(default_conf, ticker, fee, mocker) -> None:
    ...
    * 3rd trade is sold via sell-signal
    ...
    mocker.patch("traderpilot.strategy.interface.IStrategy.should_exit", should_sell_mock)
    wallets_mock = mocker.patch("traderpilot.wallets.Wallets.update")
    mocker.patch("traderpilot.wallets.Wallets.get_free", return_value=1000)
    mocker.patch("traderpilot.wallets.Wallets.check_exit_amount", return_value=True)

    traderpilot = get_patched_traderpilotbot(mocker, default_conf)
    traderpilot.strategy.order_types["stoploss_on_exchange"] = True
    # Switch ordertype to market to close trade immediately
    traderpilot.strategy.order_types["exit"] = "market"
    ...
    assert traderpilot.strategy.confirm_trade_entry.call_count == 3
    traderpilot.strategy.confirm_trade_entry.reset_mock()
    ...
    assert traderpilot.strategy.confirm_trade_entry.call_count == 0
    assert traderpilot.strategy.confirm_trade_exit.call_count == 1
    traderpilot.strategy.confirm_trade_exit.reset_mock()
...
def test_forcebuy_last_unlimited(default_conf, ticker, fee, mocker, balance_ratio, result1) -> None:
    ...
    trades = Trade.session.scalars(select(Trade)).all()
    assert len(trades) == 4
    assert traderpilot.wallets.get_trade_stake_amount("XRP/BTC", 5) == result1
    ...
    assert len(trades) == 5
    bals = traderpilot.wallets.get_all_balances()
    ...
    bals2 = traderpilot.wallets.get_all_balances()
    assert bals != bals2
    assert len(bals) == 6
    assert len(bals2) == 5
    assert "LTC" in bals
...
def test_dca_short(default_conf_usdt, ticker_usdt, fee, mocker) -> None:
    ...
    patch_get_signal(traderpilot, enter_long=False, enter_short=True)
    traderpilot.enter_positions()
...
def test_dca_order_adjust(default_conf_usdt, ticker_usdt, leverage, fee, mocker) -> None:
    ...
    patch_get_signal(traderpilot)
    traderpilot.strategy.custom_entry_price = lambda **kwargs: ticker_usdt["ask"] * 0.96
    traderpilot.strategy.leverage = MagicMock(return_value=leverage)
    traderpilot.strategy.minimal_roi = {0: 0.2}
    ...
    traderpilot.strategy.adjust_entry_price = MagicMock(return_value=1.99)
    traderpilot.process()
    trade = Trade.get_trades().first()
    assert len(trade.orders) == 2
    assert trade.has_open_orders
    ...
    traderpilot.strategy.adjust_trade_position = MagicMock(return_value=120)
    mocker.patch(f"{EXMS}._dry_is_price_crossed", return_value=False)
    ...
    traderpilot.strategy.adjust_entry_price = MagicMock(return_value=1.95)
    ...
    # Fill DCA order
    traderpilot.strategy.adjust_trade_position = MagicMock(return_value=None)
    mocker.patch(f"{EXMS}._dry_is_price_crossed", return_value=True)
    traderpilot.strategy.adjust_entry_price = MagicMock(side_effect=ValueError)
    ...
    # Full exit
    mocker.patch(f"{EXMS}._dry_is_price_crossed", return_value=False)
    traderpilot.strategy.custom_exit = MagicMock(return_value="Exit now")
    traderpilot.strategy.adjust_entry_price = MagicMock(return_value=2.02)
    traderpilot.process()
    trade = Trade.get_trades().first()
...
def test_dca_order_adjust_entry_replace_fails(
    ...
    traderpilot.strategy.adjust_trade_position = MagicMock(return_value=(20, "PeNF"))
    ...
    traderpilot.strategy.adjust_entry_price = MagicMock(return_value=2.05)
    ...
    assert traderpilot.strategy.adjust_entry_price.call_count == 2
    ...
    entry_mock = mocker.patch(
        "traderpilot.traderpilotbot.TraderpilotBot.execute_entry", return_value=False
    )
...
def test_dca_exiting(default_conf_usdt, ticker_usdt, fee, mocker, caplog, leverage) -> None:
    ...
    if not spot:
        ...
        default_conf_usdt["margin_mode"] = "isolated"
    traderpilot = get_patched_traderpilotbot(mocker, default_conf_usdt)
    assert traderpilot.trading_mode == TradingMode.FUTURES if not spot else TradingMode.SPOT
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
        get_fee=fee,
    ...
    mocker.patch(f"{EXMS}.get_max_leverage", return_value=10)
    starting_amount = traderpilot.wallets.get_total("USDT")
    ...
    assert trade.open_rate == 2.0
    assert pytest.approx(traderpilot.wallets.get_free("USDT")) == starting_amount - 60
    if spot:
        assert pytest.approx(traderpilot.wallets.get_total("USDT")) == starting_amount - 60
    else:
        assert traderpilot.wallets.get_total("USDT") == starting_amount

    # Too small size
    traderpilot.strategy.adjust_trade_position = MagicMock(return_value=-59)
    traderpilot.process()
    trade = Trade.get_trades().first()
    assert len(trade.orders) == 1
    assert pytest.approx(trade.stake_amount) == 60
    ...
    traderpilot.strategy.adjust_trade_position = MagicMock(return_value=(-20, "PES"))

    traderpilot.process()
    trade = Trade.get_trades().first()
    assert len(trade.orders) == 2
    assert trade.orders[-1].tp_order_side == "sell"
    assert trade.orders[-1].tp_order_tag == "PES"
    ...
    assert pytest.approx(traderpilot.wallets.get_free("USDT")) == expected_profit

    if spot:
        assert pytest.approx(traderpilot.wallets.get_total("USDT")) == expected_profit
    else:
        # total won't change in futures mode, only free / used will.
        assert traderpilot.wallets.get_total("USDT") == starting_amount + trade.realized_profit
    caplog.clear()

    # Sell more than what we got (we got ~20 coins left)
    # Doesn't exit, as the amount is too high.
    traderpilot.strategy.adjust_trade_position = MagicMock(return_value=-50)
    traderpilot.process()
    trade = Trade.get_trades().first()
    assert len(trade.orders) == 2
    ...
    traderpilot.strategy.adjust_trade_position = MagicMock(return_value=-trade.stake_amount)
    traderpilot.process()
    ...
    traderpilot.strategy.adjust_trade_position = MagicMock(return_value=-0.01)
    traderpilot.process()
    trade = Trade.get_trades().first()
    assert len(trade.orders) == 3
    assert trade.orders[-1].tp_order_side == "sell"
