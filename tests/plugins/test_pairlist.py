...
def test_log_cached(mocker, static_pl_conf, markets, tickers):
    ...
    traderpilot = get_patched_traderpilotbot(mocker, static_pl_conf)
    logmock = MagicMock()
    # Assign starting whitelist
    pl = traderpilot.pairlists._pairlist_handlers[0]
...
def test_load_pairlist_noexist(mocker, markets, default_conf):
    ...
    plm = PairListManager(traderpilot.exchange, default_conf, MagicMock())
    with pytest.raises(
        ...
        PairListResolver.load_pairlist(
            "NonexistingPairList", traderpilot.exchange, plm, default_conf, {}, 1
        )
...
def test_refresh_market_pair_not_in_whitelist(mocker, markets, static_pl_conf):
    ...
    mocker.patch(f"{EXMS}.markets", PropertyMock(return_value=markets))
    traderpilot.pairlists.refresh_pairlist()
    # List ordered by BaseVolume
    whitelist = ["ETH/BTC", "TKN/BTC"]
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(traderpilot.pairlists.whitelist)
    # Ensure config dict hasn't been changed
    assert (
        static_pl_conf["exchange"]["pair_whitelist"]
        == traderpilot.config["exchange"]["pair_whitelist"]
    )
...
def test_refresh_static_pairlist(mocker, markets, static_pl_conf):
    ...
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(traderpilot.pairlists.whitelist)
    assert static_pl_conf["exchange"]["pair_blacklist"] == traderpilot.pairlists.blacklist
...
def test_refresh_static_pairlist_noexist(mocker, markets, static_pl_conf, pairs, expected, caplog):
    ...
    # Ensure all except those in whitelist are removed
    assert set(expected) == set(traderpilot.pairlists.whitelist)
    assert static_pl_conf["exchange"]["pair_blacklist"] == traderpilot.pairlists.blacklist
    if not expected:
...
def test_remove_logs_for_pairs_already_in_blacklist(mocker, markets, static_pl_conf, caplog):
    ...
    for _ in range(3):
        new_whitelist = traderpilot.pairlists.verify_blacklist(
...
def test_refresh_pairlist_dynamic(mocker, shitcoinmarkets, tickers, whitelist_conf):
    ...
    traderpilot = get_patched_traderpilotbot(mocker, whitelist_conf)
    # Remock markets with shitcoinmarkets since get_patched_traderpilotbot uses the markets fixture
    mocker.patch.multiple(
        EXMS,
    ...
    traderpilot.pairlists.refresh_pairlist()
    assert whitelist == traderpilot.pairlists.whitelist
    ...
    with pytest.raises(
        ...
        PairListManager(traderpilot.exchange, whitelist_conf, MagicMock())
...
def test_refresh_pairlist_dynamic_2(mocker, shitcoinmarkets, tickers, whitelist_conf_2):
    ...
    mocker.patch.multiple(
        "traderpilot.plugins.pairlistmanager.PairListManager",
        _get_cached_tickers=MagicMock(return_value=tickers_dict),
    )
    traderpilot = get_patched_traderpilotbot(mocker, whitelist_conf_2)
    # Remock markets with shitcoinmarkets since get_patched_traderpilotbot uses the markets fixture
    mocker.patch.multiple(
        EXMS,
        markets=PropertyMock(return_value=shitcoinmarkets),
...
def test_VolumePairList_whitelist_gen(
    ...
    mocker.patch.multiple(
        "traderpilot.persistence.Trade", get_overall_performance=MagicMock(return_value=[])
    )
    ...
    if whitelist_result == "filter_at_the_beginning":
        ...
        traderpilot.pairlists.refresh_pairlist()
        whitelist = traderpilot.pairlists.whitelist
...
def test_pairlist_class(mocker, whitelist_conf, markets, pairlist):
    ...
    assert traderpilot.pairlists.name_list == [pairlist]
    assert pairlist in str(traderpilot.pairlists.short_desc())
    assert isinstance(traderpilot.pairlists.whitelist, list)
    assert isinstance(traderpilot.pairlists.blacklist, list)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["ETH/BTC", "TKN/BTC", "XLTCUSDT"], "is not tradable with Traderpilot"),
    ],
)
def test__whitelist_for_active_markets(
    ...
    # Assign starting whitelist
    pairlist_handler = traderpilot.pairlists._pairlist_handlers[0]
...
def test_volumepairlist_invalid_sortvalue(mocker, whitelist_conf):
    ...
    with pytest.raises(OperationalException, match=r"key asdf not in .*"):
        get_patched_traderpilotbot(mocker, whitelist_conf)
...
def test_volumepairlist_caching(mocker, markets, whitelist_conf, tickers):
    ...
    traderpilot = get_patched_traderpilotbot(mocker, whitelist_conf)
    assert len(traderpilot.pairlists._pairlist_handlers[0]._pair_cache) == 0
    assert tickers.call_count == 0
    traderpilot.pairlists.refresh_pairlist()
    assert tickers.call_count == 1

    assert len(traderpilot.pairlists._pairlist_handlers[0]._pair_cache) == 1
    traderpilot.pairlists.refresh_pairlist()
    assert tickers.call_count == 1
...
def test_agefilter_caching(mocker, markets, whitelist_conf_agefilter, tickers, ohlcv_history):
    with time_machine.travel("2021-09-01 05:00:00 +00:00") as t:
        ...
        traderpilot = get_patched_traderpilotbot(mocker, whitelist_conf_agefilter)
        assert traderpilot.exchange.refresh_latest_ohlcv.call_count == 0
        traderpilot.pairlists.refresh_pairlist()
        assert len(traderpilot.pairlists.whitelist) == 3
        assert traderpilot.exchange.refresh_latest_ohlcv.call_count > 0

        traderpilot.pairlists.refresh_pairlist()
        assert len(traderpilot.pairlists.whitelist) == 3
        # Call to XRP/BTC cached
        assert traderpilot.exchange.refresh_latest_ohlcv.call_count == 2
        ...
        traderpilot.pairlists.refresh_pairlist()
        assert len(traderpilot.pairlists.whitelist) == 3
        assert traderpilot.exchange.refresh_latest_ohlcv.call_count == 1
        ...
        mocker.patch(f"{EXMS}.refresh_latest_ohlcv", return_value=ohlcv_data)
        traderpilot.pairlists.refresh_pairlist()
        assert len(traderpilot.pairlists.whitelist) == 4
...
def test_rangestabilityfilter_caching(
    ...
    assert traderpilot.exchange.refresh_latest_ohlcv.call_count == 0
    traderpilot.pairlists.refresh_pairlist()
    assert len(traderpilot.pairlists.whitelist) == expected_length
    assert traderpilot.exchange.refresh_latest_ohlcv.call_count > 0

    previous_call_count = traderpilot.exchange.refresh_latest_ohlcv.call_count
    traderpilot.pairlists.refresh_pairlist()
    assert len(traderpilot.pairlists.whitelist) == expected_length
    # Should not have increased since first call.
    assert traderpilot.exchange.refresh_latest_ohlcv.call_count == previous_call_count
...
def test_pricefilter_desc(
    ...
    if desc_expected is not None:
        traderpilot = get_patched_traderpilotbot(mocker, whitelist_conf)
        short_desc = str(traderpilot.pairlists.short_desc())
...
def test_performance_filter(
    ...
    traderpilot = get_patched_traderpilotbot(mocker, allowlist_conf)
    ...
    mocker.patch.multiple(
        "traderpilot.persistence.Trade",
        get_overall_performance=MagicMock(return_value=overall_performance),
    )
    traderpilot.pairlists.refresh_pairlist()
    allowlist = traderpilot.pairlists.whitelist
...
def test_MarketCapPairList_filter(
    ...
    mocker.patch(
        "traderpilot.plugins.pairlist.MarketCapPairList.FtCoinGeckoApi.get_coins_categories_list",
        return_value=[
            {"category_id": "layer-1"},
    ...
    gcm_mock = mocker.patch(
        "traderpilot.plugins.pairlist.MarketCapPairList.FtCoinGeckoApi.get_coins_markets",
