...
def test_protectionmanager(mocker, default_conf):
    ...
    for handler in traderpilot.protections._protection_handlers:
...
def test_stoploss_guard(mocker, default_conf, fee, caplog, is_short):
    ...
    assert not traderpilot.protections.global_stop()
    ...
    assert traderpilot.protections.global_stop()
    ...
    traderpilot.protections.global_stop(end_time)
...
def test_stoploss_guard_perpair(mocker, default_conf, fee, caplog, only_per_pair, only_per_side):
    ...
    message = r"Trading stopped due to .*"
    pair = "XRP/BTC"
    assert not traderpilot.protections.stop_per_pair(pair)
    assert not traderpilot.protections.global_stop()
    assert not log_has_re(message, caplog)
    caplog.clear()
    ...
    # 3 Trades closed - but the 2nd has been closed too long ago.
    assert not traderpilot.protections.stop_per_pair(pair)
    assert traderpilot.protections.global_stop() != only_per_pair
    ...
    traderpilot.protections.stop_per_pair(pair)
    assert traderpilot.protections.global_stop() != only_per_pair
...
def test_CooldownPeriod(mocker, default_conf, fee, caplog):
    ...
    message = r"Trading stopped due to .*"
    assert not traderpilot.protections.global_stop()
    assert not traderpilot.protections.stop_per_pair("XRP/BTC")
    ...
    assert not traderpilot.protections.global_stop()
    assert traderpilot.protections.stop_per_pair("XRP/BTC")
    ...
    assert traderpilot.protections.stop_per_pair("ETH/BTC")
...
def test_LowProfitPairs(mocker, default_conf, fee, caplog, only_per_side):
    ...
    Trade.commit()
    assert traderpilot.protections.stop_per_pair("XRP/BTC") != only_per_side
    ...
    assert traderpilot.protections.global_stop() != only_per_side
    assert traderpilot.protections.stop_per_pair("XRP/BTC") != only_per_side
...
def test_protection_manager_desc(
    ...
    short_desc = str(traderpilot.protections.short_desc())
