...
def test_rpc_trade_status(default_conf, ticker, fee, mocker) -> None:
    ...
    # Fill open order ...
    traderpilotbot.manage_open_orders()
    trades = Trade.get_open_trades()
    traderpilotbot.exit_positions(trades)
...
def test__rpc_timeunit_profit(
    ...
    traderpilotbot = get_patched_traderpilotbot(mocker, default_conf_usdt)
...
def test_rpc_delete_trade(mocker, default_conf, fee, markets, caplog, is_short):
    ...
    traderpilotbot = get_patched_traderpilotbot(mocker, default_conf)
    traderpilotbot.strategy.order_types["stoploss_on_exchange"] = True
    create_mock_trades(fee, is_short)
    rpc = RPC(traderpilotbot)
    ...
    trades[2].orders.append(
        Order(
            ...
            tp_pair=trades[2].pair,
            tp_is_open=True,
            tp_amount=trades[2].amount,
            tp_price=trades[2].stop_loss,
            order_id="102",
            status="open",
        )
    )
...
def test_rpc_balance_handle(default_conf_usdt, mocker, tickers, proxy_coin, margin_mode):
    ...
    mocker.patch("traderpilot.rpc.rpc.CryptoToFiatConverter._find_price", return_value=1.2)
    mocker.patch("traderpilot.rpc.telegram.Telegram", MagicMock())
    mocker.patch.multiple(
        EXMS,
...
def test_rpc_force_exit(default_conf, ticker, fee, mocker) -> None:
    ...
    traderpilotbot.config["max_open_trades"] = 3
    traderpilotbot.enter_positions()
...
def test_rpc_force_entry(mocker, default_conf, ticker, fee, limit_buy_order_open) -> None:
    ...
    traderpilotbot.config["max_open_trades"] = 5
    ...
    traderpilotbot.strategy.position_adjustment_enable = True
    ...
    pair = "XRP/BTC"
    traderpilotbot = get_patched_traderpilotbot(mocker, default_conf)
    traderpilotbot.config["stake_amount"] = 0
    patch_get_signal(traderpilotbot)
    rpc = RPC(traderpilotbot)
    pair = "TKN/BTC"
...
def test_rpc_edge_enabled(mocker, edge_conf) -> None:
    ...
    traderpilotbot = get_patched_traderpilotbot(mocker, edge_conf)
