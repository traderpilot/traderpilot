...
def ccxt_exceptionhandlers(
    ...
    **kwargs,
    ...
    with patch("traderpilot.exchange.common.time.sleep"):
...
async def async_ccxt_exception(
    ...
    with patch("traderpilot.exchange.common.asyncio.sleep", get_mock_coro(None)):
        with pytest.raises(DDosProtection):
...
def test_validate_order_time_in_force(default_conf, mocker, caplog):
    ...
    ex._tp_has.update({"order_time_in_force": ["GTC", "FOK", "IOC"]})
    ex.validate_order_time_in_force(tif2)
...
def test_validate_traderai_compat(default_conf, mocker, caplog):
    caplog.set_level(logging.INFO)
    # Test kraken - as it doesn't support historic trades data.
    ex = get_patched_exchange(mocker, default_conf, exchange="kraken")
    mocker.patch(f"{EXMS}.exchange_has", return_value=True)

    default_conf["traderai"] = {"enabled": False}
    ex.validate_traderai(default_conf)

    default_conf["traderai"] = {"enabled": True}
...
def test_validate_timeframes_failed(default_conf, mocker):
    ...
    with pytest.raises(
        ConfigurationError, match=r"Timeframes < 1m are currently not supported by Traderpilot."
    ):
        Exchange(default_conf)
...
def test_validate_required_startup_candles(default_conf, mocker, caplog):
    ...
    # Emulate kraken mode
    ex._tp_has["ohlcv_has_history"] = False
    with pytest.raises(
        OperationalException,
...
def test_exchange_has(default_conf, mocker):
    ...
    exchange._tp_has["exchange_has_overrides"] = {"deadbeef": True}
    assert exchange.exchange_has("deadbeef")
...
def test_create_order(default_conf, mocker, side, ordertype, rate, marketprice, exchange_name):
    ...
    price_req = exchange._tp_has.get("marketOrderRequiresPrice", False)
...
def test_refresh_latest_trades(
    ...
    exchange._tp_has["exchange_has_overrides"]["fetchTrades"] = True
...
async def test__async_kucoin_get_candle_history(default_conf, mocker, caplog):
    from traderpilot.exchange.common import _reset_logging_mixin
...
async def test___async_get_candle_history_sort(default_conf, mocker, exchange_name):
    ...
    exchange._api_async.fetch_ohlcv = get_mock_coro(ohlcv)
    # Reset sort mock
    sort_mock = mocker.patch("traderpilot.exchange.sorted", MagicMock(side_effect=sort_data))
    # Test the OHLCV data sort
    res = await exchange._async_get_candle_history(
        "ETH/BTC", default_conf["timeframe"], CandleType.SPOT
    )
...
async def test__async_get_trade_history_id(default_conf, mocker, exchange_name, fetch_trades_result):
    ...
    assert exchange._tp_has["trades_pagination_arg"] in fetch_trades_cal[1][1]["params"]
...
def test_cancel_stoploss_order_with_result(default_conf, mocker, exchange_name):
    default_conf["dry_run"] = False
    mock_prefix = "traderpilot.exchange.gate.Gate"
    if exchange_name == "okx":
        mock_prefix = "traderpilot.exchange.okx.Okx"
...
def test_fetch_order(default_conf, mocker, exchange_name, caplog):
    ...
    exchange = get_patched_exchange(mocker, default_conf, api_mock, exchange=exchange_name)
    with patch("traderpilot.exchange.common.time.sleep") as tm:
        with pytest.raises(InvalidOrderException):
            exchange.fetch_order(order_id="_", pair="TKN/BTC")
...
def test_merge_tp_has_dict(default_conf, mocker):
    mocker.patch.multiple(
        EXMS,
    ...
    ex = Exchange(default_conf)
    assert ex._tp_has == Exchange._tp_has_default
    ...
    conf = copy.deepcopy(default_conf)
    conf["exchange"]["_tp_has_params"] = {"DeadBeef": 20, "stoploss_on_exchange": False}
    # Use settings from configuration (overriding stoploss_on_exchange)
    ex = Binance(conf)
    assert ex._tp_has != Exchange._tp_has_default
    assert not ex._tp_has["stoploss_on_exchange"]
    assert ex._tp_has["DeadBeef"] == 20
...
def test_ohlcv_candle_limit(default_conf, mocker, exchange_name):
    ...
    exchange = get_patched_exchange(mocker, default_conf, exchange=exchange_name)
    timeframes = ("1m", "5m", "1h")
    expected = exchange._tp_has["ohlcv_candle_limit"]
    for timeframe in timeframes:
        # if 'ohlcv_candle_limit_per_timeframe' in exchange._tp_has:
        # expected = exchange._tp_has['ohlcv_candle_limit_per_timeframe'][timeframe]
