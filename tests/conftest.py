...
EXMS = "traderpilot.exchange.exchange.Exchange"
...
def patched_configuration_load_config_file(mocker, config) -> None:
    mocker.patch(
        "traderpilot.configuration.load_config.load_config_file", lambda *args, **kwargs: config
    )
...
def patch_exchange(
    ...
    # Temporary patch ...
    mocker.patch("traderpilot.exchange.bybit.Bybit.cache_leverage_tiers")
    ...
    if mock_supported_modes:
        mocker.patch(
            f"traderpilot.exchange.{exchange}.{exchange.capitalize()}"
...
def patch_wallet(mocker, free=999.9) -> None:
    mocker.patch("traderpilot.wallets.Wallets.get_free", MagicMock(return_value=free))


def patch_whitelist(mocker, conf) -> None:
    mocker.patch(
        "traderpilot.traderpilotbot.TraderpilotBot._refresh_active_whitelist",
...
def patch_edge(mocker) -> None:
    ...
    mocker.patch("traderpilot.edge.Edge.calculate", MagicMock(return_value=True))


# Functions for recurrent object patching


def patch_traderpilotbot(mocker, config) -> None:
    """
    ...
    mocker.patch("traderpilot.traderpilotbot.RPCManager._init", MagicMock())
    mocker.patch("traderpilot.traderpilotbot.RPCManager.send_msg", MagicMock())
    patch_whitelist(mocker, config)
    mocker.patch("traderpilot.traderpilotbot.ExternalMessageConsumer")
    mocker.patch("traderpilot.configuration.config_validation._validate_consumers")


def get_patched_traderpilotbot(mocker, config) -> TraderpilotBot:
    """
    This function patches _init_modules() to not call dependencies
    :param mocker: a Mocker object to apply patches
    :param config: Config to pass to the bot
    :return: TraderpilotBot
    """
    patch_traderpilotbot(mocker, config)
    return TraderpilotBot(config)
...
def patch_get_signal(
    ...
    traderpilot.strategy.get_entry_signal = patched_get_entry_signal
    ...
    # returns (enter, exit)
    traderpilot.strategy.get_exit_signal = patched_get_exit_signal

    traderpilot.exchange.refresh_latest_ohlcv = lambda p: None
...
def patch_gc(mocker) -> None:
    mocker.patch("traderpilot.main.gc_set_threshold")
...
def patch_coingecko(mocker) -> None:
    ...
    mocker.patch.multiple(
        "traderpilot.rpc.fiat_convert.FtCoinGeckoApi",
