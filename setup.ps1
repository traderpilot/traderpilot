...
$RequirementFiles = @("requirements.txt", "requirements-dev.txt", "requirements-hyperopt.txt", "requirements-traderai.txt", "requirements-traderai-rl.txt", "requirements-plot.txt")
$VenvName = ".venv"
...
function Main {
  ...
  # Install traderpilot from setup using the virtual environment's Python
  Write-Log "Installing traderpilot from setup..."
  pip install -e . 2>&1 | Out-File $LogFilePath -Append
  if ($LASTEXITCODE -ne 0) {
    Write-Log "Failed to install traderpilot." -Level 'ERROR'
    Exit-Script -exitCode 1
  }

  Write-Log "Installing traderUI..."
  python traderpilot install-ui 2>&1 | Out-File $LogFilePath -Append
  if ($LASTEXITCODE -ne 0) {
    Write-Log "Failed to install traderUI." -Level 'ERROR'
    Exit-Script -exitCode 1
  }
