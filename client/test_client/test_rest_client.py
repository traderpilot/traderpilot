...
from traderpilot_client import TpRestClient
from traderpilot_client.client import add_arguments, main_exec
...
def get_rest_client():
    client = TpRestClient("http://localhost:8080", "traderpilotr", "password")
...
def test_TpRestClient_init():
    client = TpRestClient("http://localhost:8080", "traderpilotr", "password")
    assert client is not None
    assert client._serverurl == "http://localhost:8080"
    assert client._session is not None
    assert client._session.auth is not None
    assert client._session.auth == ("traderpilotr", "password")


@pytest.mark.parametrize("method", ["GET", "POST", "DELETE"])
def test_TpRestClient_call(method):
...
def test_TpRestClient_call_invalid(caplog):
...
def test_TpRestClient_call_explicit_methods(method, args, kwargs):
    client, mock = get_rest_client()
...
def test_client(mocker, capsys, caplog):
    ...
    mock = mocker.patch("traderpilot_client.client.TpRestClient._call")
...
def test_client_argparsing(mocker, params, expected_args, expected_kwargs, caplog):
    mocked_method = params.split(" ")[0]
    mocker.patch("traderpilot_client.client.load_config", return_value={}, autospec=True)
    mm = mocker.patch(
        f"traderpilot_client.client.TpRestClient.{mocked_method}", return_value={}, autospec=True
    )
