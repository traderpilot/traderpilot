...
from traderpilot.persistence.trade_model import Order, Trade
...
def migrate_orders_table(engine, table_back_name: str, cols_order: list):
    tp_fee_base = get_column_def(cols_order, "tp_fee_base", "null")
    average = get_column_def(cols_order, "average", "null")
    stop_price = get_column_def(cols_order, "stop_price", "null")
    funding_fee = get_column_def(cols_order, "funding_fee", "0.0")
    tp_amount = get_column_def(cols_order, "tp_amount", "coalesce(amount, 0.0)")
    tp_price = get_column_def(cols_order, "tp_price", "coalesce(price, 0.0)")
    tp_cancel_reason = get_column_def(cols_order, "tp_cancel_reason", "null")
    tp_order_tag = get_column_def(cols_order, "tp_order_tag", "null")
    ...
    with engine.begin() as connection:
        connection.execute(
            text(
                f"""
            insert into orders (id, tp_trade_id, tp_order_side, tp_pair, tp_is_open, order_id,
            status, symbol, order_type, side, price, amount, filled, average, remaining, cost,
            stop_price, order_date, order_filled_date, order_update_date, tp_fee_base, funding_fee,
            tp_amount, tp_price, tp_cancel_reason, tp_order_tag
            )
            select id, tp_trade_id, tp_order_side, tp_pair, tp_is_open, order_id,
            status, symbol, order_type, side, price, amount, filled, {average} average, remaining,
            cost, {stop_price} stop_price, order_date, order_filled_date,
            order_update_date, {tp_fee_base} tp_fee_base, {funding_fee} funding_fee,
            {tp_amount} tp_amount, {tp_price} tp_price, {tp_cancel_reason} tp_cancel_reason,
            {tp_order_tag} tp_order_tag
...
def check_migrate(engine, decl_base, previous_tables) -> None:
    ...
    if not has_column(cols_trades, "precision_mode_price"):
        # if not has_column(cols_orders, "tp_order_tag"):
        migrating = True
        logger.info(
    ...
    if "orders" not in previous_tables and "trades" in previous_tables:
        raise OperationalException(
            ...
            "Please update to traderpilot 2022.3 to migrate this database or "
