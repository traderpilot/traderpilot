...
from traderpilot.exchange.exchange_types import CcxtOrder
from traderpilot.leverage import interest
from traderpilot.misc import safe_value_fallback
from traderpilot.persistence.base import ModelBase, SessionType
from traderpilot.persistence.custom_data import CustomDataWrapper, _CustomData
from traderpilot.util import FtPrecise, dt_from_ts, dt_now, dt_ts, dt_ts_none
...
class Order(ModelBase):
    ...
    __table_args__ = (UniqueConstraint("tp_pair", "order_id", name="_order_pair_order_id"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    tp_trade_id: Mapped[int] = mapped_column(Integer, ForeignKey("trades.id"), index=True)
    ...
    tp_order_side: Mapped[str] = mapped_column(String(25), nullable=False)
    tp_pair: Mapped[str] = mapped_column(String(25), nullable=False)
    tp_is_open: Mapped[bool] = mapped_column(nullable=False, default=True, index=True)
    tp_amount: Mapped[float] = mapped_column(Float(), nullable=False)
    tp_price: Mapped[float] = mapped_column(Float(), nullable=False)
    tp_cancel_reason: Mapped[str] = mapped_column(String(CUSTOM_TAG_MAX_LENGTH), nullable=True)
    ...
    tp_fee_base: Mapped[float | None] = mapped_column(Float(), nullable=True)
    tp_order_tag: Mapped[str | None] = mapped_column(String(CUSTOM_TAG_MAX_LENGTH), nullable=True)
    ...
    def safe_amount(self) -> float:
        return self.amount or self.tp_amount

    @property
    def safe_placement_price(self) -> float:
        """Price at which the order was placed"""
        return self.price or self.stop_price or self.tp_price

    @property
    def safe_price(self) -> float:
        return self.average or self.price or self.stop_price or self.tp_price
    ...
    def safe_fee_base(self) -> float:
        return self.tp_fee_base or 0.0
    ...
    def __repr__(self):
        return (
            f"Order(id={self.id}, trade={self.tp_trade_id}, order_id={self.order_id}, "
    ...
    def to_ccxt_object(self, stopPriceName: str = "stopPrice") -> dict[str, Any]:
        ...
        if self.tp_order_side == "stoploss":
            order.update(
                {
                    stopPriceName: self.stop_price,
                    "tp_order_type": "stoploss",
                }
            )
    ...
    def to_json(self, entry_side: str, minified: bool = False) -> dict[str, Any]:
        ...
        resp = {
            ...
            "tp_order_side": self.tp_order_side,
            "order_filled_timestamp": dt_ts_none(self.order_filled_utc),
            " tp_is_entry": self.tp_order_side == entry_side,
            "tp_order_tag": self.tp_order_tag,
        }
        if not minified:
            resp.update(
                {
    ...
    def close_bt_order(self, close_date: datetime, trade: "LocalTrade"):
        ...
        if self.tp_order_side == trade.entry_side and self.price:
    ...
    def parse_from_ccxt_object(
        ...
        o = cls(
            ...
            tp_order_side=side,
            tp_pair=pair,
            tp_amount=amount or order.get("amount", None) or 0.0,
            tp_price=price or order.get("price", None),
        )
...
class LocalTrade:
    ...
    def sl_orders(self) -> list[Order]:
        ...
        """
        return [o for o in self.orders if o.tp_order_side in ["stoploss"]]

    @property
    def open_orders_ids(self) -> list[str]:
        open_orders_ids_wo_sl = [
            oo.order_id for oo in self.open_orders if oo.tp_order_side not in ["stoploss"]
        ]
    ...
    def update_trade(self, order: Order, recalculating: bool = False) -> None:
        ...
        if order.tp_order_side != "stoploss":
        ...
        if order.tp_order_side == self.entry_side:
        ...
        elif order.tp_order_side == self.exit_side:
            if self.is_open:
                payment = "BUY" if self.is_short else "SELL"
                # * On margin shorts, you buy a little bit more than the amount (amount + interest)
                logger.info(f"{order_type}_{payment} has been fulfilled for {self}.")

        elif order.tp_order_side == "stoploss" and order.status not in ("open",):
    ...
    def fully_canceled_entry_order_count(self) -> int:
        ...
        return len(
            [
                ...
                if o.tp_order_side == self.entry_side
    ...
    def canceled_exit_order_count(self) -> int:
        ...
        return len(
            [
                o
                for o in self.orders
                if o.tp_order_side == self.exit_side and o.status in CANCELED_EXCHANGE_STATES
            ]
        )
    ...
    def calc_close_trade_value(self, rate: float, amount: float | None = None) -> float:
        ...
        if trading_mode == TradingMode.SPOT:
            ...
            raise OperationalException(
                f"{self.trading_mode} trading is not yet available using traderpilot"
            )
    ...
    def recalc_trade_from_orders(self, *, is_closing: bool = False):
        ...
        for i, o in enumerate(self.orders):
            ...
            is_exit = o.tp_order_side != self.entry_side
    ...
    def select_order(
        ...
        :param order_side: tp_order_side of the order (either 'buy', 'sell' or 'stoploss')
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        for order in data["orders"]:
            order_obj = Order(
                amount=order["amount"],
                tp_amount=order["amount"],
                tp_order_side=order["tp_order_side"],
                tp_pair=order["pair"],
                tp_is_open=order["is_open"],
                ...
                tp_order_tag=order.get("tp_order_tag", None),
            )
