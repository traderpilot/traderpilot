...
class _CustomData(ModelBase):
    ...
    __table_args__ = (UniqueConstraint("tp_trade_id", "cd_key", name="_trade_id_cd_key"),)

    id = mapped_column(Integer, primary_key=True)
    tp_trade_id = mapped_column(Integer, ForeignKey("trades.id"), index=True)
    ...
    def __repr__(self):
        ...
        return (
            ...
            + f"value={self.cd_value}, trade_id={self.tp_trade_id}, created={create_time}, "
    ...
    def query_cd(cls, key: str | None = None, trade_id: int | None = None) -> Sequence["_CustomData"]:
        ...
        if trade_id is not None:
            filters.append(_CustomData.tp_trade_id == trade_id)
        if key is not None:
...
class CustomDataWrapper:
    ...
    def delete_custom_data(trade_id: int) -> None:
        _CustomData.session.query(_CustomData).filter(_CustomData.tp_trade_id == trade_id).delete()
    ...
    def get_custom_data(*, trade_id: int, key: str | None = None) -> list[_CustomData]:
        if CustomDataWrapper.use_db:
            filters = [
                _CustomData.tp_trade_id == trade_id,
            ]
            if key is not None:
                filters.append(_CustomData.cd_key.ilike(key))
            ...
            filtered_custom_data = [
                ...
                if (data_entry.tp_trade_id == trade_id)
            ]
            if key is not None:
                filtered_custom_data = [
                    data_entry
