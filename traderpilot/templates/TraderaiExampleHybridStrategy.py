...
from traderpilot.strategy import IntParameter, IStrategy, merge_informative_pair  # noqa


logger = logging.getLogger(__name__)


class TraderaiExampleHybridStrategy(IStrategy):
    """
    Example of a hybrid TraderAI strat, designed to illustrate how a user may employ
    TraderAI to bolster a typical Traderpilot strategy.

    Launching this strategy would be:

    traderpilot trade --strategy TraderaiExampleHybridStrategy --strategy-path traderpilot/templates
    --traderaimodel CatboostClassifier --config config_examples/config_traderai.example.json
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:  # noqa: C901
        ...
        # TA indicators to combine with the Traderai targets
        # RSI
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        df.loc[
            (
                ...
                & (df["do_predict"] == 1)  # Make sure Traderai is confident in the prediction
                &
                # Only enter trade if Traderai thinks the trend is in this direction
                (df["&s-up_or_down"] == "up")
            ),
            "enter_long",
        ] = 1
