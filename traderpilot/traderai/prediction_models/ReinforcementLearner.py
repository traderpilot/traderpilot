...
from traderpilot.traderai.RL.BaseEnvironment import BaseEnvironment
from traderpilot.traderai.RL.BaseReinforcementLearningModel import BaseReinforcementLearningModel
...
class ReinforcementLearner(BaseReinforcementLearningModel):
    ...
    Save the file to `user_data/traderaimodels`, then run it with:
    ...
    def fit(self, data_dictionary: dict[str, Any], dk: TraderaiDataKitchen, **kwargs):
        ...
        :param dk: TraderaiDatakitchen = data kitchen for current pair.
        :return:
        model Any = trained model to be used for inference in dry/live/backtesting
        """
        train_df = data_dictionary["train_features"]
        total_timesteps = self.traderai_info["rl_config"]["train_cycles"] * len(train_df)
        ...
        if dk.pair not in self.dd.model_dictionary or not self.continual_learning:
            model = self.MODELCLASS(
                ...
                **self.traderai_info.get("model_training_parameters", {}),
            )
        else:
            logger.info(
