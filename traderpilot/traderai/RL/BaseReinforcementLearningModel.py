...
class BaseReinforcementLearningModel(ITraderaiModel):
    """
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.max_threads = min(
            self.traderai_info["rl_config"].get("cpu_count", 1),
            max(int(self.max_system_threads / 2), 1),
        )
        th.set_num_threads(self.max_threads)
        self.reward_params = self.traderai_info["rl_config"]["model_reward_parameters"]
        ...
        self.rl_config = self.traderai_info["rl_config"]
        ...
        self.policy_type = self.traderai_info["rl_config"]["policy_type"]
    ...
    def unset_outlier_removal(self):
        ...
        """
        if self.tp_params.get("use_SVM_to_remove_outliers", False):
            self.tp_params.update({"use_SVM_to_remove_outliers": False})
            logger.warning("User tried to use SVM with RL. Deactivating SVM.")
        if self.tp_params.get("use_DBSCAN_to_remove_outliers", False):
            self.tp_params.update({"use_DBSCAN_to_remove_outliers": False})
            logger.warning("User tried to use DBSCAN with RL. Deactivating DBSCAN.")
        if self.tp_params.get("DI_threshold", False):
            self.tp_params.update({"DI_threshold": False})
            logger.warning("User tried to use DI_threshold with RL. Deactivating DI_threshold.")
        if self.traderai_info["data_split_parameters"].get("shuffle", False):
            self.traderai_info["data_split_parameters"].update({"shuffle": False})
    ...
    def set_train_and_eval_environments(
        ...
        :param dk: TraderaiDataKitchen = the datakitchen for the current pair
        """
    ...
    @abstractmethod
    def fit(self, data_dictionary: dict[str, Any], dk: TraderaiDataKitchen, **kwargs):
        """
    ...
    def rl_model_predict(
        self, dataframe: DataFrame, dk: TraderaiDataKitchen, model: Any
    ) -> DataFrame:
        """
        A helper function to make predictions in the Reinforcement learning module.
        :param dataframe: DataFrame = the dataframe of features to make the predictions on
        :param dk: TraderaiDatakitchen = data kitchen for the current pair
    ...
    def build_ohlc_price_dataframes(
        self, data_dictionary: dict, pair: str, dk: TraderaiDataKitchen
    ...
    def drop_ohlc_from_df(self, df: DataFrame, dk: TraderaiDataKitchen):
        """
    ...
    def load_model_from_disk(self, dk: TraderaiDataKitchen) -> Any:
        """
