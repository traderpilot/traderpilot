...
def download_all_data_for_training(dp: DataProvider, config: Config) -> None:
    ...
    refresh_backtest_ohlcv_data(
        dp._exchange,
        pairs=all_pairs,
        timeframes=config["traderai"]["feature_parameters"].get("include_timeframes"),
...
def get_required_data_timerange(config: Config) -> TimeRange:
    ...
    for auto data-download in TraderAI
    """
    ...
    indicator_periods = config["traderai"]["feature_parameters"]["indicator_periods_candles"]
    ...
    logger.info(f"TraderAI auto-downloader using {max_period} startup candles.")
...
def plot_feature_importance(
    model: Any, pair: str, dk: TraderaiDataKitchen, count_max: int = 25
) -> None:
    """
    ...
    """
    from traderpilot.plot.plotting import go, make_subplots, store_plot_file

    # Extract feature importance from model
    models = {}
    if "TraderaiMultiOutputRegressor" in str(model.__class__):
    ...
    for label in models:
        ...
        label = label.replace("&", "").replace("%", "")  # escape two TraderAI specific characters
...
def get_timerange_backtest_live_models(config: Config) -> str:
    ...
    """
    dk = TraderaiDataKitchen(config)
    models_path = dk.get_full_models_path(config)
    dd = TraderaiDataDrawer(models_path, config)
...
def get_tb_logger(model_type: str, path: Path, activate: bool) -> Any:
    if model_type == "pytorch" and activate:
        from traderpilot.traderai.tensorboard import TBLogger

        return TBLogger(path, activate)
    else:
        from traderpilot.traderai.tensorboard.base_tensorboard import BaseTensorboardLogger
