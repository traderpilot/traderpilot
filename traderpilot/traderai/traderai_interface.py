...
from traderpilot.constants import DOCS_LINK, Config
...
class ITraderaiModel(ABC):
    """
    Class containing all tools for training and prediction in the strategy.
    Base*PredictionModels inherit from this class.

    Record of contribution:
    TraderAI was developed by a group of individuals who all contributed specific skillsets to the
    project.
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.traderai_info: dict[str, Any] = config["traderai"]
        self.data_split_parameters: dict[str, Any] = config.get("traderai", {}).get(
            "data_split_parameters", {}
        )
        self.model_training_parameters: dict[str, Any] = config.get("traderai", {}).get(
            "model_training_parameters", {}
        )
        self.identifier: str = self.traderai_info.get("identifier", "no_id_provided")
        self.retrain = False
        self.first = True
        self.set_full_path()
        self.save_backtest_models: bool = self.traderai_info.get("save_backtest_models", True)
        if self.save_backtest_models:
            logger.info("Backtesting module configured to save all models.")

        self.dd = TraderaiDataDrawer(Path(self.full_path), self.config)
        # set current candle to arbitrary historical date
        self.current_candle: datetime = datetime.fromtimestamp(637887600, tz=timezone.utc)
        self.dd.current_candle = self.current_candle
        self.scanning = False
        self.tp_params = self.traderai_info["feature_parameters"]
        self.corr_pairlist: list[str] = self.tp_params.get("include_corr_pairlist", [])
        self.keras: bool = self.traderai_info.get("keras", False)
        if self.keras and self.tp_params.get("DI_threshold", 0):
            self.tp_params["DI_threshold"] = 0
            logger.warning("DI threshold is not configured for Keras models yet. Deactivating.")

        self.CONV_WIDTH = self.traderai_info.get("conv_width", 1)
        ...
        self.continual_learning = self.traderai_info.get("continual_learning", False)
        self.plot_features = self.tp_params.get("plot_feature_importances", 0)
        ...
        self.model: Any = None
        if self.tp_params.get("principal_component_analysis", False) and self.continual_learning:
            self.tp_params.update({"principal_component_analysis": False})
            logger.warning("User tried to use PCA with continual learning. Deactivating PCA.")
        self.activate_tensorboard: bool = self.traderai_info.get("activate_tensorboard", True)
    ...
    def assert_config(self, config: Config) -> None:
        if not config.get("traderai", {}):
            raise OperationalException("No traderai parameters found in configuration file.")

    def start(self, dataframe: DataFrame, metadata: dict, strategy: IStrategy) -> DataFrame:
        """
        Entry point to the TraderaiModel from a specific pair, it will train a new model if
        ...
        if self.live:
            ...
            self.dk = TraderaiDataKitchen(self.config, self.live, metadata["pair"])
        ...
        # TraderAI slides the window and sequentially builds the backtesting results before returning
        # the concatenated results for the full backtesting period back to the strategy.
        else:
            self.dk = TraderaiDataKitchen(self.config, self.live, metadata["pair"])
            if not self.config.get("traderai_backtest_live_models", False):
    ...
    def shutdown(self):
        ...
        """
        logger.info("Stopping TraderAI")
        self._stop_event.set()

        self.data_provider = None
        self._on_stop()

        if self.traderai_info.get("wait_for_training_iteration_on_reload", True):
    ...
    def _start_scanning(self, strategy: IStrategy) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            dk = TraderaiDataKitchen(self.config, self.live, pair)
            (
                retrain,
            ...
            if retrain:
                ...
                self.dd.save_historic_predictions_to_disk()
                if self.traderai_info.get("write_metrics_to_disk", False):
                    self.dd.save_metric_tracker_to_disk()

    def start_backtesting(
        self, dataframe: DataFrame, metadata: dict, dk: TraderaiDataKitchen, strategy: IStrategy
    ) -> TraderaiDataKitchen:
        """
        ...
        following the training window). TraderAI slides the window and sequentially builds
        ...
        :param dk: TraderaiDataKitchen = Data management/analysis tool associated to present pair only
        :param strategy: Strategy to train on
        :return:
            TraderaiDataKitchen = Data management/analysis tool associated to present pair only
        """
    ...
    def start_live(
        self, dataframe: DataFrame, metadata: dict, strategy: IStrategy, dk: TraderaiDataKitchen
    ) -> TraderaiDataKitchen:
        """
        ...
        dk: TraderaiDataKitchen = Data management/analysis tool associated to present pair only
        :returns:
        dk: TraderaiDataKitchen = Data management/analysis tool associated to present pair only
        """
        ...
        if not strategy.process_only_new_candles:
            raise OperationalException(
                "You are trying to use a TraderAI strategy with "
                "process_only_new_candles = False. This is not supported "
                "by TraderAI, and it is therefore aborting."
            )
    ...
    def build_strategy_return_arrays(
        self, dataframe: DataFrame, dk: TraderaiDataKitchen, pair: str, trained_timestamp: int
    ) -> None:
        ...
        if pair not in self.dd.model_return_values:
            ...
            # allows TraderUI to show full return values.
        ...
        if self.traderai_info.get("fit_live_predictions_candles", 0) and self.live:
    ...
    def check_if_feature_list_matches_strategy(self, dk: TraderaiDataKitchen) -> None:
        """
        Ensure user is passing the proper feature set if they are reusing an `identifier` pointing
        to a folder holding existing models.
        :param dataframe: DataFrame = strategy provided dataframe
        :param dk: TraderaiDataKitchen = non-persistent data container/analyzer for
        ...
        if dk.training_features_list != feature_list:
            raise OperationalException(
                ...
                "model. In case of --strategy-list, please be aware that TraderAI "
                "requires all strategies to maintain identical "
                "feature_engineering_* functions"
            )

    def define_data_pipeline(self, threads=-1) -> Pipeline:
        tp_params = self.traderai_info["feature_parameters"]
        pipe_steps = [
            ("const", ds.VarianceThreshold(threshold=0)),
            ("scaler", SKLearnWrapper(MinMaxScaler(feature_range=(-1, 1)))),
        ]

        if tp_params.get("principal_component_analysis", False):
            pipe_steps.append(("pca", ds.PCA(n_components=0.999)))
            pipe_steps.append(
                ("post-pca-scaler", SKLearnWrapper(MinMaxScaler(feature_range=(-1, 1))))
            )

        if tp_params.get("use_SVM_to_remove_outliers", False):
            svm_params = tp_params.get("svm_params", {"shuffle": False, "nu": 0.01})
            pipe_steps.append(("svm", ds.SVMOutlierExtractor(**svm_params)))

        di = tp_params.get("DI_threshold", 0)
        if di:
            pipe_steps.append(("di", ds.DissimilarityIndex(di_threshold=di, n_jobs=threads)))

        if tp_params.get("use_DBSCAN_to_remove_outliers", False):
            pipe_steps.append(("dbscan", ds.DBSCAN(n_jobs=threads)))

        sigma = self.traderai_info["feature_parameters"].get("noise_standard_deviation", 0)
        if sigma:
    ...
    def model_exists(self, dk: TraderaiDataKitchen) -> bool:
        """
    ...
    def extract_data_and_train_model(
        ...
        :param dk: TraderaiDataKitchen = non-persistent data container for current coin/loop
    ...
    def set_initial_historic_predictions(
        self, pred_df: DataFrame, dk: TraderaiDataKitchen, pair: str, strat_df: DataFrame
    ) -> None:
        """
        ...
        data. After that, TraderAI will append new real predictions to the
        ...
        :param pred_df: DataFrame = the dataframe containing the predictions coming
            out of a model
        :param dk: TraderaiDataKitchen = object containing methods for data analysis
        ...
        if self.traderai_info["feature_parameters"].get("DI_threshold", 0) > 0:
    ...
    def fit_live_predictions(self, dk: TraderaiDataKitchen, pair: str) -> None:
        """
        ...
        num_candles = self.traderai_info.get("fit_live_predictions_candles", 100)
    ...
    def inference_timer(self, do: Literal["start", "stop"] = "start", pair: str = ""):
        """
        Timer designed to track the cumulative time spent in TraderAI for one pass through
    ...
    def cache_corr_pairlist_dfs(self, dataframe: DataFrame, dk: TraderaiDataKitchen) -> DataFrame:
        """
    ...
    def track_current_candle(self):
        ...
        equivalent to the latest candle seen by TraderAI. If not, it
    ...
    def ensure_data_exists(
        ...
        """
        if self.config.get("traderai_backtest_live_models", False) and len_dataframe_backtest == 0:
            logger.info(
    ...
    def backtesting_fit_live_predictions(self, dk: TraderaiDataKitchen):
        """
        ...
        """
        fit_live_predictions_candles = self.traderai_info.get("fit_live_predictions_candles", 0)
    ...
    def start_backtesting_from_historic_predictions(
        self, dataframe: DataFrame, metadata: dict, dk: TraderaiDataKitchen
    ) -> TraderaiDataKitchen:
        """
        :param dataframe: DataFrame = strategy passed dataframe
        :param metadata: Dict = pair metadata
    ...
    # See traderai/prediction_models/CatboostPredictionModel.py for an example.

    @abstractmethod
    def train(self, unfiltered_df: DataFrame, pair: str, dk: TraderaiDataKitchen, **kwargs) -> Any:
        """
    ...
    @abstractmethod
    def fit(self, data_dictionary: dict[str, Any], dk: TraderaiDataKitchen, **kwargs) -> Any:
        """
        Most regressors use the same function names and arguments e.g. user
        can drop in LGBMRegressor in place of CatBoostRegressor and all data
        management will be properly handled by Traderai.
    ...
    @abstractmethod
    def predict(
        self, unfiltered_df: DataFrame, dk: TraderaiDataKitchen, **kwargs
        ...
        :do_predict: np.array of 1s and 0s to indicate places where traderai needed to remove
        data (NaNs) or felt uncertain about data (i.e. SVM and/or DI index)
        """

    # deprecated functions
    def data_cleaning_train(self, dk: TraderaiDataKitchen, pair: str):
        """
        ...
        logger.warning(
            ...
            f"{DOCS_LINK}/strategy_migration/#traderai-new-data-pipeline"
        )
    ...
    def data_cleaning_predict(self, dk: TraderaiDataKitchen, pair: str):
        """
        throw deprecation warning if this function is called
        """
        logger.warning(
        ...
        if self.traderai_info.get("DI_threshold", 0) > 0:
