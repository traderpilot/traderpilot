...
from traderpilot.data.converter import reduce_dataframe_footprint
from traderpilot.exceptions import OperationalException
from traderpilot.exchange import timeframe_to_seconds
from traderpilot.strategy import merge_informative_pair
...
class TraderaiDataKitchen:
    """
    Class designed to analyze data for a single pair. Employed by the ITraderaiModel class.
    ...
    def __init__(
        ...
        self.config = config
        self.traderai_config: dict[str, Any] = config["traderai"]
        ...
        self.live = live
        self.pair = pair
        self.keras: bool = self.traderai_config.get("keras", False)
        self.set_all_pairs()
        self.backtest_live_models = config.get("traderai_backtest_live_models", False)
        ...
        if not self.live:
            ...
            if not self.backtest_live_models:
                self.full_timerange = self.create_fulltimerange(
                    self.config["timerange"], self.traderai_config.get("train_period_days", 0)
                )
                (self.training_timeranges, self.backtesting_timeranges) = self.split_timerange(
                    self.full_timerange,
                    config["traderai"]["train_period_days"],
        ...
        if not self.traderai_config.get("data_kitchen_thread_count", 0):
            ...
            self.thread_count = self.traderai_config["data_kitchen_thread_count"]
    ...
    def make_train_test_datasets(
        ...
        """
        feat_dict = self.traderai_config["feature_parameters"]

        if "shuffle" not in self.traderai_config["data_split_parameters"]:
            self.traderai_config["data_split_parameters"].update({"shuffle": False})
        ...
        if self.traderai_config.get("data_split_parameters", {}).get("test_size", 0.1) != 0:
            (
                train_features,
                test_features,
                train_labels,
                ...
                labels,
                weights,
                **self.config["traderai"]["data_split_parameters"],
            )
        else:
        ...
        if self.traderai_config["feature_parameters"].get("reverse_train_test_order", False):
    ...
    def set_weights_higher_recent(self, num_weights: int) -> npt.ArrayLike:
        ...
        """
        wfactor = self.config["traderai"]["feature_parameters"]["weight_factor"]
    ...
    def get_predictions_to_append(
        ...
        if self.traderai_config["feature_parameters"].get("DI_threshold", 0) > 0:
    ...
    def create_fulltimerange(self, backtest_tr: str, backtest_period_days: int) -> str:
        ...
        if backtest_timerange.stopts == 0:
            ...
            raise OperationalException(
                "TraderAI backtesting does not allow open ended timeranges. "
    ...
    def check_if_model_expired(self, trained_timestamp: int) -> bool:
        ...
        max_time = self.traderai_config.get("expiration_hours", 0)
        if max_time > 0:
    ...
    def check_if_new_training_required(
        ...
        timeframes = self.traderai_config["feature_parameters"].get("include_timeframes")
        ...
        if trained_timestamp != 0:
            ...
            retrain = elapsed_time > self.traderai_config.get("live_retrain_hours", 0)
            if retrain:
                trained_timerange.startts = int(
                    time - self.traderai_config.get("train_period_days", 0) * SECONDS_IN_DAY
                )
                ...
                data_load_timerange.startts = int(
                    time
                    - self.traderai_config.get("train_period_days", 0) * SECONDS_IN_DAY
                    - additional_seconds
                )
    ...
    def set_all_pairs(self) -> None:
        self.all_pairs = copy.deepcopy(
            self.traderai_config["feature_parameters"].get("include_corr_pairlist", [])
        )
    ...
    def extract_corr_pair_columns_from_populated_indicators(
        ...
        pairs = self.traderai_config["feature_parameters"].get("include_corr_pairlist", [])
    ...
    def populate_features(
        ...
        for tf in tfs:
            ...
            for t in self.traderai_config["feature_parameters"]["indicator_periods_candles"]:
    ...
    def use_strategy_to_populate_indicators(  # noqa: C901
        ...
        if not new_version:
            raise OperationalException(
                ...
                f"{DOCS_LINK}/strategy_migration/#traderai-strategy \n"
                "And the feature_engineering_* documentation: \n"
                f"{DOCS_LINK}/traderai-feature-engineering/"
            )
        ...
        corr_pairs: list[str] = self.traderai_config["feature_parameters"].get(
        ...
        if self.live:
            dataframe = strategy.set_traderai_targets(dataframe.copy(), metadata=metadata)
    ...
    def get_full_models_path(self, config: Config) -> Path:
        """
        Returns default TraderAI model path
        :param config: Configuration dictionary
        """
        traderai_config: dict[str, Any] = config["traderai"]
        return Path(config["user_data_dir"] / "models" / str(traderai_config.get("identifier")))
    ...
    def buffer_timerange(self, timerange: TimeRange):
        ...
        will be NaN and TraderAI will automatically cut those off of the training
        dataset.
        """
        buffer = self.traderai_config["feature_parameters"]["buffer_train_data_candles"]
        if buffer:
    ...
    def normalize_data(self, data_dictionary: dict) -> dict[Any, Any]:
        ...
        """
        logger.warning(
            f"Your custom ITraderaiModel relies on the deprecated"
