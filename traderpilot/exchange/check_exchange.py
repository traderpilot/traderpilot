...
from traderpilot.exchange import available_exchanges, is_exchange_known_ccxt, validate_exchange
from traderpilot.exchange.common import MAP_EXCHANGE_CHILDCLASS, SUPPORTED_EXCHANGES
...
def check_exchange(config: Config, check_for_bad: bool = True) -> bool:
    """
    Check if the exchange name in the config file is supported by Traderpilot
    :param check_for_bad: if True, check the exchange against the list of known 'bad'
                          exchanges
    :return: False if exchange is 'bad', i.e. is known to work with the bot with
             critical issues or does not work at all, crashes, etc. True otherwise.
             raises an exception if the exchange if not supported by ccxt
             and thus is not known for the Traderpilot at all.
    """
    ...
    if not exchange:
        raise OperationalException(
            ...
            f"The following exchanges are available for Traderpilot: "
