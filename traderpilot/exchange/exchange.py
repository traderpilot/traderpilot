...
from traderpilot.exchange.common import (
    API_FETCH_ORDER_RETRY_COUNT,
    remove_exchange_credentials,
    retrier,
    retrier_async,
)
from traderpilot.exchange.exchange_types import (
    CcxtBalances,
    CcxtOrder,
    CcxtPosition,
    FtHas,
...
from traderpilot.util import dt_from_ts, dt_now
from traderpilot.util.datetime_helpers import dt_humanize_delta, dt_ts, format_ms_time
...
class Exchange:
    ...
    # This defines defaults, which can be selectively overridden by subclasses using _tp_has
    # or by specifying them in the configuration.
    _tp_has_default: FtHas = {
    ...
    def __init__(
        ...
        *,
        ...
        # Deep merge tp_has with default tp_has options
        self._tp_has = deep_merge_dicts(self._tp_has, deepcopy(self._tp_has_default))
        if self.trading_mode == TradingMode.FUTURES:
            self._tp_has = deep_merge_dicts(self._tp_has_futures, self._tp_has)
        if exchange_conf.get("_tp_has_params"):
            self._tp_has = deep_merge_dicts(exchange_conf.get("_tp_has_params"), self._tp_has)
            logger.info("Overriding exchange._tp_has with config params, result: %s", self._tp_has)

        # Assign this directly for easy access
        self._ohlcv_partial_candle = self._tp_has["ohlcv_partial_candle"]

        self._max_trades_limit = self._tp_has["trades_limit"]

        self._trades_pagination = self._tp_has["trades_pagination"]
        self._trades_pagination_arg = self._tp_has["trades_pagination_arg"]
        ...
        self._has_watch_ohlcv = self.exchange_has("watchOHLCV") and self._tp_has["ws_enabled"]
        if (
    ...
    def _ccxt_config(self) -> dict:
        ...
        if self.trading_mode == TradingMode.MARGIN:
            ...
            return {"options": {"defaultType": self._tp_has["ccxt_futures_name"]}}
        else:
            return {}
    ...
    def ohlcv_candle_limit(
        ...
        fallback_val = self._tp_has.get("ohlcv_candle_limit")
        if candle_type == CandleType.FUNDING_RATE:
            fallback_val = self._tp_has.get("funding_fee_candle_limit", fallback_val)
        return int(
            self._tp_has.get("ohlcv_candle_limit_per_timeframe", {}).get(timeframe, str(fallback_val))
        )
    ...
    def market_is_future(self, market: dict[str, Any]) -> bool:
        return (
            market.get(self._tp_has["ccxt_futures_name"], False) is True
    ...
    def market_is_tradable(self, market: dict[str, Any]) -> bool:
        """
        Check if the market symbol is tradable by Traderpilot.
    ...
    def _order_contracts_to_amount(self, order: CcxtOrder) -> CcxtOrder:
        if "symbol" in order and order["symbol"] is not None:
            ...
            if contract_size != 1:
                for prop in self._tp_has.get("order_props_in_contracts", []):
    ...
    def reload_markets(self, force: bool = False, *, load_leverage_tiers: bool = True) -> None:
        ...
        try:
            ...
            if is_initial and self._tp_has["needs_trading_fees"]:
    ...
    def validate_timeframes(self, timeframe: str | None) -> None:
        ...
        if (
            ...
            raise ConfigurationError("Timeframes < 1m are currently not supported by Traderpilot.")
    ...
    def validate_stop_ordertypes(self, order_types: dict) -> None:
        ...
        """
        if order_types.get("stoploss_on_exchange") and not self._tp_has.get(
            "stoploss_on_exchange", False
        ):
            raise ConfigurationError(f"On exchange stoploss is not supported for {self.name}.")
        if self.trading_mode == TradingMode.FUTURES:
            price_mapping = self._tp_has.get("stop_price_type_value_mapping", {}).keys()
            if (
    ...
    def validate_pricing(self, pricing: dict) -> None:
        ...
        if not pricing.get("use_order_book", False) and (
            not self.exchange_has("fetchTicker") or not self._tp_has["tickers_have_price"]
        ):
    ...
    def validate_order_time_in_force(self, order_time_in_force: dict) -> None:
        ...
        """
        if any(
            v.upper() not in self._tp_has["order_time_in_force"]
    ...
    def validate_orderflow(self, exchange: dict) -> None:
        if exchange.get("use_public_trades", False) and (
            not self.exchange_has("fetchTrades") or not self._tp_has["trades_has_history"]
        ):
            raise ConfigurationError(
                f"Trade data not available for {self.name}. Can't use orderflow feature."
            )

    def validate_traderai(self, config: Config) -> None:
        traderai_enabled = config.get("traderai", {}).get("enabled", False)
        if traderai_enabled and not self._tp_has["ohlcv_has_history"]:
            raise ConfigurationError(
                f"Historic OHLCV data not available for {self.name}. Can't use traderAI."
            )
    ...
    def validate_required_startup_candles(self, startup_candles: int, timeframe: str) -> int:
        ...
        if self._tp_has["ohlcv_has_history"]:
    ...
    def validate_trading_mode_and_margin_mode(
        ...
        """
        Checks if traderpilot can perform trades using the configured
        trading mode(Margin, Futures) and MarginMode(Cross, Isolated)
        Throws OperationalException:
            If the trading_mode/margin_mode type are not supported by traderpilot on this exchange
        """
        if trading_mode != TradingMode.SPOT and (
            ...
            raise OperationalException(
                f"Traderpilot does not support {mm_value} {trading_mode} on {self.name}"
            )

    def get_option(self, param: str, default: Any | None = None) -> Any:
        """
        Get parameter value from _tp_has
        """
        return self._tp_has.get(param, default)
    ...
    def exchange_has(self, endpoint: str) -> bool:
        ...
        :return: bool
        """
        if endpoint in self._tp_has.get("exchange_has_overrides", {}):
            return self._tp_has["exchange_has_overrides"][endpoint]
    ...
    def create_dry_run_order(
        ...
        if stop_loss:
            ...
            dry_order[self._tp_has["stop_price_prop"]] = dry_order["price"]
            # Workaround to avoid filling stoploss orders immediately
            dry_order["tp_order_type"] = "stoploss"
        ...
        if dry_order["type"] == "market" and not dry_order.get("tp_order_type"):
    ...
    def check_dry_limit_order_filled(
        ...
        if (
            ...
            and not order.get("tp_order_type")
        ):
            pair = order["symbol"]
    ...
    def fetch_dry_run_order(self, order_id) -> CcxtOrder:
        ...
        try:
            ...
            from traderpilot.persistence import Order

            order = Order.order_by_id(order_id)
            if order:
                ccxt_order = order.to_ccxt_object(self._tp_has["stop_price_prop"])
    ...
    def stoploss_adjust(self, stop_loss: float, order: CcxtOrder, side: str) -> bool:
        ...
        """
        if not self._tp_has.get("stoploss_on_exchange"):
            raise OperationalException(f"stoploss is not implemented for {self.name}.")
        price_param = self._tp_has["stop_price_prop"]
    ...
    def _get_stop_order_type(self, user_order_type) -> tuple[str, str]:
        available_order_Types: dict[str, str] = self._tp_has["stoploss_order_types"]
    ...
    def _get_stop_params(self, side: BuySell, ordertype: str, stop_price: float) -> dict:
        ...
        params.update({self._tp_has["stop_price_param"]: stop_price})
        return params
    ...
    def create_stoploss(
        ...
        requires `_tp_has['stoploss_order_types']` to be set as a dict mapping limit and market
        ...
        """
        if not self._tp_has["stoploss_on_exchange"]:
            raise OperationalException(f"stoploss is not implemented for {self.name}.")
        ...
        try:
            ...
            if self.trading_mode == TradingMode.FUTURES:
                ...
                if "stoploss_price_type" in order_types and "stop_price_type_field" in self._tp_has:
                    price_type = self._tp_has["stop_price_type_value_mapping"][
                        order_types.get("stoploss_price_type", PriceType.LAST)
                    ]
                    params[self._tp_has["stop_price_type_field"]] = price_type
    ...
    def get_conversion_rate(self, coin: str, currency: str) -> float | None:
        ...
        if (proxy_coin := self._tp_has["proxy_coin_mapping"].get(coin, None)) is not None:
            coin = proxy_coin
        if (proxy_currency := self._tp_has["proxy_coin_mapping"].get(currency, None)) is not None:
    ...
    def fetch_l2_order_book(self, pair: str, limit: int = 100) -> OrderBook:
        ...
        limit1 = self.get_next_limit_in_list(
            limit, self._tp_has["l2_limit_range"], self._tp_has["l2_limit_range_required"]
        )
        try:
    ...
    def _build_coroutine(
        ...
        if not since_ms and (self._tp_has["ohlcv_require_since"] or not_all_data):
    ...
    async def _async_get_candle_history(
        ...
        try:
            ...
            params = deepcopy(self._tp_has.get("ohlcv_params", {}))
            candle_limit = self.ohlcv_candle_limit(
    ...
    def _now_is_time_to_refresh_trades(
        ...
        full_candle = (
            int(timeframe_to_next_date(timeframe, dt_from_ts(pair_last_refreshed)).timestamp()) * 1000
        )
        now = dt_ts()
    ...
    async def _async_get_trade_history_id(
        ...
        has_overlap = self._tp_has.get("trades_pagination_overlap", True)
    ...
    def _set_leverage(
        ...
        if self._config["dry_run"] or not self.exchange_has("setLeverage"):
            ...
            return
        if self._tp_has.get("floor_leverage", False) is True:
    ...
    def _fetch_and_calculate_funding_fees(
        ...
        timeframe = self._tp_has["mark_ohlcv_timeframe"]
        timeframe_ff = self._tp_has["funding_fee_timeframe"]
        mark_price_type = CandleType.from_string(self._tp_has["mark_ohlcv_price"])
    ...
    def dry_run_liquidation_price(
        ...
        if self.trading_mode == TradingMode.FUTURES and self.margin_mode == MarginMode.ISOLATED:
            if market["inverse"]:
                raise OperationalException("Traderpilot does not yet support inverse contracts")
            ...
            raise OperationalException(
                "Traderpilot only supports isolated futures for leverage trading"
            )
