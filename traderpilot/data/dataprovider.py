...
from traderpilot.data.history import get_datahandler, load_pair_history
from traderpilot.enums import CandleType, RPCMessageType, RunMode, TradingMode
from traderpilot.exceptions import ExchangeError, OperationalException
from traderpilot.exchange import Exchange, timeframe_to_prev_date, timeframe_to_seconds
from traderpilot.exchange.exchange_types import OrderBook
from traderpilot.misc import append_candles_to_dataframe
from traderpilot.rpc import RPCManager
from traderpilot.rpc.rpc_types import RPCAnalyzedDFMsg
from traderpilot.util import PeriodicCache
...
class DataProvider:
    ...
    def get_required_startup(self, timeframe: str) -> int:
        traderai_config = self._config.get("traderai", {})
        if not traderai_config.get("enabled", False):
            return self._config.get("startup_candle_count", 0)
        else:
            startup_candles = self._config.get("startup_candle_count", 0)
            indicator_periods = traderai_config["feature_parameters"]["indicator_periods_candles"]
            # make sure the startupcandles is at least the set maximum indicator periods
            self._config["startup_candle_count"] = max(startup_candles, max(indicator_periods))
            tf_seconds = timeframe_to_seconds(timeframe)
            train_candles = traderai_config["train_period_days"] * 86400 / tf_seconds
            total_candles = int(self._config["startup_candle_count"] + train_candles)
            logger.info(
                f"Increasing startup_candle_count for traderai on {timeframe} to {total_candles}"
            )
