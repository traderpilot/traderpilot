"""
Traderpilot is the main module of this bot. It contains the class Traderpilot()
"""
...
from traderpilot.configuration import validate_config_consistency
from traderpilot.constants import BuySell, Config, EntryExecuteMode, ExchangeConfig, LongShort
from traderpilot.data.converter import order_book_to_dataframe
...
from traderpilot.misc import safe_value_fallback, safe_value_fallback2
from traderpilot.mixins import LoggingMixin
from traderpilot.persistence import Order, PairLocks, Trade, init_db
...
from traderpilot.rpc.rpc_types import (
    ProfitLossStr,
    RPCCancelMsg,
    RPCEntryMsg,
    RPCExitCancelMsg,
...
from traderpilot.util import FtPrecise, MeasureTime, dt_from_ts
...
class TraderpilotBot(LoggingMixin):
    """
    Traderpilot is the main class of the bot.
    ...
    def __init__(self, config: Config) -> None:
        ...
        # initialization, even before Traderpilotbot has a chance to start its throttling,
        # so anything in the Traderpilotbot instance should be ready (initialized), including
    ...
    def process(self) -> None:
        ...
        # Without this, traderpilot may try to recreate stoploss_on_exchange orders
        ...
        for order in orders:
            try:
                fo = self.exchange.fetch_order_or_stoploss_order(
                    order.order_id, order.tp_pair, order.tp_order_side == "stoploss"
                )
                if not order.trade:
                    ...
                    logger.warning(
                        ...
                        f"The expected trade ID is {order.tp_trade_id}. Ignoring this order."
                    )
                    continue
                self.update_trade_state(
                    ...
                    order.order_id,
                    fo,
                    stoploss_order=(order.tp_order_side == "stoploss"),
                )
        ...
        for order in trade.orders:
            ...
            try:
                ...
                if fo:
                    ...
                    self.update_trade_state(
                        trade, order.order_id, fo, stoploss_order=order.tp_order_side == "stoploss"
                    )
        ...
        try:
            ...
            if not trade.is_open:
                ...
                self.order_close_notify(
                    trade,
                    order_obj,
                    order_obj.tp_order_side == "stoploss",
            ...
            # catching https://github.com/traderpilot/traderpilot/issues/9025
    ...
    def execute_entry(
        ...
        *,
        ...
        order_obj.tp_order_tag = enter_tag
        order_id = order["id"]
    ...
    def _notify_enter(
        ...
        if not fill and trade.nr_of_successful_entries > 0:
            ...
            stake_amount += sum(
                o.stake_amount for o in trade.open_orders if o.tp_order_side == trade.entry_side
            )
            ...
            for open_order in trade.open_orders:
                ...
                if not_closed:
                    if fully_cancelled or (
                        open_order
                        and self.strategy.tp_check_timed_out(
    ...
    def handle_cancel_enter(
        ...
        if order["status"] not in constants.NON_OPEN_EXCHANGE_STATES:
            ...
            corder = order
            if order_obj.tp_cancel_reason is None:
                order_obj.tp_cancel_reason = constants.CANCEL_REASON["CANCELLED_ON_EXCHANGE"]

        logger.info(f"{side} order {order_obj.tp_cancel_reason} for {trade}.")
        ...
        if isclose(filled_amount, 0.0, abs_tol=constants.MATH_CLOSE_PREC):
            ...
            if open_order_count < 1 and trade.nr_of_successful_entries == 0 and not replacing:
                ...
                trade.delete()
                order_obj.tp_cancel_reason += f", {constants.CANCEL_REASON['FULLY_CANCELLED']}"
            else:
            ...
            order_obj.tp_cancel_reason += f", {constants.CANCEL_REASON['PARTIALLY_FILLED']}"

        self.wallets.update()
        self._notify_enter_cancel(
            trade, order_type=self.strategy.order_types["entry"], reason=order_obj.tp_cancel_reason
        )
    ...
    def handle_cancel_exit(
        ...
        self._notify_exit_cancel(
            trade,
            order_type=self.strategy.order_types["exit"],
            reason=order_obj.tp_cancel_reason,
            order_id=order["id"],
            sub_trade=trade.amount != order["amount"],
        )
        return cancelled
    ...
    def execute_trade_exit(
        ...
        *,
        ...
        order_obj.tp_order_tag = exit_reason
        trade.orders.append(order_obj)
    ...
    def _update_trade_after_fill(self, trade: Trade, order: Order, send_msg: bool) -> Trade:
        if order.status in constants.NON_OPEN_EXCHANGE_STATES:
            ...
            if order.tp_order_side == trade.entry_side:
                if send_msg:
            ...
            if (
                order.tp_order_side == trade.entry_side
                ...
                if self.strategy.use_custom_stoploss:
                    ...
                    self.strategy.tp_stoploss_adjust(
    ...
    def apply_fee_conditional(
        ...
        if order_obj.tp_order_side == trade.exit_side or order_obj.tp_order_side == "stoploss":
            # check against remaining amount!
            amount_ = trade.amount - amount

        if trade.nr_of_successful_entries >= 1 and order_obj.tp_order_side == trade.entry_side:
    ...
    def get_real_amount(self, trade: Trade, order: CcxtOrder, order_obj: Order) -> float | None:
        ...
        if self.exchange.order_has_fee(order):
            ...
            logger.info(
                f"Fee for Trade {trade} [{order_obj.tp_order_side}]: "
