...
from traderpilot.constants import DATETIME_PRINT_FORMAT, Config, IntOrInf, LongShort
from traderpilot.data import history
from traderpilot.data.btanalysis import find_existing_backtest_stats, trade_list_to_dataframe
from traderpilot.data.converter import trim_dataframe, trim_dataframes
...
from traderpilot.optimize.bt_progress import BTProgress
...
from traderpilot.persistence import (
    CustomDataWrapper,
    LocalTrade,
    Order,
    PairLocks,
...
class Backtesting:
    ...
    def __init__(self, config: Config, exchange: Exchange | None = None) -> None:
        ...
        if self.config.get("strategy_list"):
            if self.config.get("traderai", {}).get("enabled", False):
                logger.warning(
                    "Using --strategy-list with TraderAI REQUIRES all strategies "
        ...
        if self.config.get("traderai_backtest_live_models", False):
            from traderpilot.traderai.utils import get_timerange_backtest_live_models
        ...
        if self.config.get("traderai", {}).get("enabled", False):
            # For TraderAI, increase the required_startup to includes the training data
    ...
    def _get_ohlcv_as_lists(self, processed: dict[str, DataFrame]) -> dict[str, tuple]:
        ...
        for pair in processed.keys():
            ...
            df_analyzed = self.strategy.tp_advise_signals(pair_data, {"pair": pair})
    ...
    def _get_close_rate_for_roi(
        ...
        if roi is not None and roi_entry is not None:
            ...
            if trade_dur == 0 and (
                ...
                # details: https: // github.com/traderpilot/traderpilot/issues/6261
    ...
    def _try_close_open_order(
        ...
        """
        if order and self._get_order_filled(order.tp_price, row):
            ...
            if self.margin_mode == MarginMode.CROSS or not (
                order.tp_order_side == trade.exit_side and order.safe_amount == trade.amount
            ):
            ...
            if not (order.tp_order_side == trade.exit_side and order.safe_amount == trade.amount):
                self._call_adjust_stop(current_date, trade, order.tp_price)
            return True
        return False
    ...
    def _process_exit_order(
        ...
        if self._try_close_open_order(order, trade, current_time, row):
            ...
            if sub_trade:
                ...
                trade.close(order.tp_price, show_msg=False)
        ...
        if exit_.exit_flag:
            ...
            if exit_.exit_type in (
                ...
                if order_type == "limit":
                    ...
                    # traderpilot does not support this in live, and the order would fill immediately
                    if trade.is_short:
    ...
    def get_valid_price_and_stake(
        ...
        if order_type == "limit":
            ...
            # which traderpilot does not support in live.
    ...
    def _enter_trade(
        ...
        if stake_amount and (not min_stake_amount or stake_amount >= min_stake_amount):
            ...
            order = Order(
                ...
                order_id=str(self.order_id_counter),
                symbol=trade.pair,
                tp_order_side=trade.entry_side,
    ...
    def check_order_cancel(
        ...
        """
        timedout = self.strategy.tp_check_timed_out(
    ...
    def check_order_replace(self, trade: LocalTrade, order: Order, current_time, row: tuple) -> bool:
        ...
        if order.side == trade.entry_side and current_time > order.order_date_utc:
            requested_rate = strategy_safe_wrapper(
                self.strategy.adjust_entry_price, default_retval=order.tp_price
            )(
                trade=trade,  # type: ignore[arg-type]
                order=order,
                pair=trade.pair,
                ...
                current_order_rate=order.tp_price,
                entry_tag=trade.enter_tag,
                side=trade.trade_direction,
            )  # default value is current order price

            # cancel existing order whenever a new rate is requested (or None)
            if requested_rate == order.tp_price:
            ...
            if requested_rate:
                self._enter_trade(
                    ...
                    requested_stake=(order.safe_remaining * order.tp_price / trade.leverage),
