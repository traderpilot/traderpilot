...
from traderpilot.optimize.base_analysis import BaseAnalysis, VarHolder
...
class LookaheadAnalysis(BaseAnalysis):
    ...
    def prepare_data(self, varholder: VarHolder, pairs_to_load: list[DataFrame]):
        if "traderai" in self.local_config and "identifier" in self.local_config["traderai"]:
            # purge previous data if the traderai model is defined
            ...
            path_to_current_identifier = Path(
                ...
                f"{self.local_config['traderai']['identifier']}"
            ).resolve()
    ...
    def start(self) -> None:
        ...
        if found_signals >= self.targeted_trade_amount:
            logger.info(
                f"Found {found_signals} trades, " f"calculating {self.targeted_trade_amount} trades."
            )
