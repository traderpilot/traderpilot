...
from traderpilot.exchange.exchange_types import Ticker
    ...
    def _validate_pair(self, pair: str, ticker: Ticker | None) -> bool:
        ...
        if self._min_price != 0:
            if price < self._min_price:
                self.log_once(
                    f"Removed {pair} from whitelist, " f"because last price < {self._min_price:.8f}",
                    logger.info,
                )
                return False
        ...
        if self._max_price != 0:
            if price > self._max_price:
                self.log_once(
                    f"Removed {pair} from whitelist, " f"because last price > {self._max_price:.8f}",
                    logger.info,
                )
                return False
