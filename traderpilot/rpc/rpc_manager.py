...
from traderpilot.enums import NO_ECHO_MESSAGES, RPCMessageType
from traderpilot.rpc import RPC, RPCHandler
...
class RPCManager:
    ...
    def __init__(self, traderpilot) -> None:
        ...
        self._rpc = RPC(traderpilot)
        config = traderpilot.config
        # Enable telegram
        if config.get("telegram", {}).get("enabled", False):
            logger.info("Enabling rpc.telegram ...")
            from traderpilot.rpc.telegram import Telegram
    ...
    def startup_messages(self, config: Config, pairlist, protections) -> None:
        ...
        if len(protections.name_list) > 0:
            ...
            self.send_msg({"type": RPCMessageType.STARTUP, "status": f"Using Protections: \n{prots}"})
