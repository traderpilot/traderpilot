...
from traderpilot.constants import DUST_PER_COIN, Config
from traderpilot.enums import MarketDirection, RPCMessageType, SignalDirection, TradingMode
from traderpilot.exceptions import OperationalException
from traderpilot.misc import chunks, plural
from traderpilot.persistence import Trade
from traderpilot.rpc import RPC, RPCException, RPCHandler
from traderpilot.rpc.rpc_types import RPCEntryMsg, RPCExitMsg, RPCOrderMsg, RPCSendMsg
...
class Telegram(RPCHandler):
    ...
    def _add_analyzed_candle(self, pair: str) -> str:
        ...
        if candle_val != "off":
            if candle_val == "ohlc":
                analyzed_df, _ = self._rpc._traderpilot.dataprovider.get_analyzed_dataframe(
    ...
    def _prepare_order_details(self, filled_orders: list, quote_currency: str, is_open: bool):
        ...
        for order in filled_orders:
            ...
            if order["is_open"] is True:
                continue
            order_nr += 1
            wording = "Entry" if order[" tp_is_entry"] else "Exit"
    ...
    async def _status_msg(self, update: Update, context: CallbackContext) -> None:
        ...
        for r in results:
            ...
            r["num_entries"] = len([o for o in r["orders"] if o[" tp_is_entry"]])
            r["num_exits"] = len(
                [
                    o
                    for o in r["orders"]
                    if not o[" tp_is_entry"] and not o["tp_order_side"] == "stoploss"
                ]
            )
    ...
    async def _help(self, update: Update, context: CallbackContext) -> None:
        ...
        if self._rpc._traderpilot.trading_mode != TradingMode.SPOT:
            force_enter_text += (
    ...
    async def _version(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        strategy_version = self._rpc._traderpilot.strategy.version()
    ...
    async def _show_config(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        val = RPC._rpc_show_config(self._config, self._rpc._traderpilot.state)
