...
from traderpilot.constants import CANCEL_REASON, DEFAULT_DATAFRAME_COLUMNS, Config
from traderpilot.data.history import load_data
from traderpilot.data.metrics import DrawDownResult, calculate_expectancy, calculate_max_drawdown
...
from traderpilot.exceptions import ExchangeError, PricingError
from traderpilot.exchange import timeframe_to_minutes, timeframe_to_msecs
from traderpilot.exchange.exchange_utils import price_to_precision
from traderpilot.loggers import bufferHandler
from traderpilot.persistence import KeyStoreKeys, KeyValueStore, PairLocks, Trade
from traderpilot.persistence.models import PairLock
from traderpilot.plugins.pairlist.pairlist_helpers import expand_pairlist
from traderpilot.rpc.fiat_convert import CryptoToFiatConverter
from traderpilot.rpc.rpc_types import RPCSendMsg
from traderpilot.util import (
    decimals_per_coin,
    dt_from_ts,
    dt_now,
    dt_ts_def,
...
from traderpilot.util.datetime_helpers import dt_humanize_delta
from traderpilot.wallets import PositionWallet, Wallet
...
class RPC:
    ...
    if TYPE_CHECKING:
        ...
        _traderpilot: TraderpilotBot

    def __init__(self, traderpilot) -> None:
        """
        Initializes all enabled rpc modules
        :param traderpilot: Instance of a traderpilot bot
        :return: None
        """
        self._traderpilot = traderpilot
        self._config: Config = traderpilot.config
    ...
    def _rpc_show_config(
        ...
        val = {
            ...
            "bot_name": config.get("bot_name", "traderpilot"),
        ...
        if not trades:
            ...
            for trade in trades:
                ...
                oo_details_lst = [
                    ...
                    if oo.tp_order_side not in ["stoploss"]
                ]
                ...
                if trade.is_open:
                    try:
                        current_rate: float = self._traderpilot.exchange.get_rate(
                ...
                if not isnan(current_profit_abs) and self._fiat_converter:
                    current_profit_fiat = self._fiat_converter.convert_amount(
                        current_profit_abs,
                        self._traderpilot.config["stake_currency"],
                        self._traderpilot.config["fiat_display_currency"],
                    )
        ...
        for trade in self._rpc_trade_status():
            ...
            if orders:
                active_order_side = ".".join(
                    "*" if (o.get("is_open") and o.get(" tp_is_entry")) else "**"
                    for o in orders
                    if o.get("is_open")
                )
    ...
    def _rpc_timeunit_profit(
        ...
        daily_stake = self._traderpilot.wallets.get_total_stake_amount()
    ...
    def _rpc_trade_statistics(
        ...
        for trade in trades:
            ...
            if not trade.is_open:
                ...
                if len(trade.select_filled_orders(trade.entry_side)) == 0:
                    ...
                    continue
                try:
                    current_rate = self._traderpilot.exchange.get_rate(
                        trade.pair, side="exit", is_short=trade.is_short, refresh=False
                    )
        ...
        starting_balance = self._traderpilot.wallets.get_starting_balance()
    ...
    def __balance_get_est_stake(
        ...
        if coin == self._traderpilot.exchange.get_proxy_coin():
            ...
            try:
                rate: float | None = self._traderpilot.exchange.get_conversion_rate(
                    coin, stake_currency
                )
                if rate:
    ...
    def _rpc_balance(self, stake_currency: str, fiat_display_currency: str) -> dict:
        ...
        self._traderpilot.wallets.update(require_update=False)
        starting_capital = self._traderpilot.wallets.get_starting_balance()
        starting_cap_fiat = (
            self._fiat_converter.convert_amount(
        ...
        coin: str
        balance: Wallet
        for coin, balance in self._traderpilot.wallets.get_all_balances().items():
            if not balance.total and not balance.free:
                continue

            trade = (
                open_assets.get(coin, None)
                if self._traderpilot.trading_mode != TradingMode.FUTURES
                else None
            )
            is_stake_currency = coin == self._traderpilot.exchange.get_proxy_coin()
            is_bot_managed = is_stake_currency or trade is not None
            trade_amount = trade.amount if trade else 0
            if is_stake_currency:
                trade_amount = self._traderpilot.wallets.get_available_stake_amount()
        ...
        for symbol, position in self._traderpilot.wallets.get_all_positions().items():
        ...
        return {
            ...
            "note": "Simulated balances" if self._traderpilot.config["dry_run"] else "",
        }
    ...
    def _rpc_start(self) -> dict[str, str]:
        ...
        self._traderpilot.state = State.RUNNING
        return {"status": "starting trader ..."}

    def _rpc_stop(self) -> dict[str, str]:
        """Handler for stop"""
        if self._traderpilot.state == State.RUNNING:
            self._traderpilot.state = State.STOPPED
    ...
    def _rpc_reload_config(self) -> dict[str, str]:
        ...
        self._traderpilot.state = State.RELOAD_CONFIG
    ...
    def _rpc_stopentry(self) -> dict[str, str]:
        ...
        if self._traderpilot.state == State.RUNNING:
            ...
            self._traderpilot.config["max_open_trades"] = 0
            self._traderpilot.strategy.max_open_trades = 0
    ...
    def _rpc_reload_trade_from_exchange(self, trade_id: int) -> dict[str, str]:
        ...
        self._traderpilot.handle_onexchange_order(trade)
    ...
    def __exec_force_exit(
        ...
        for oo in trade.open_orders:
            ...
            order = self._traderpilot.exchange.fetch_order(oo.order_id, trade.pair)

            if order["side"] == trade.entry_side:
                fully_canceled = self._traderpilot.handle_cancel_enter(
            ...
            if order["side"] == trade.exit_side:
                ...
                self._traderpilot.handle_cancel_exit(trade, order, oo, CANCEL_REASON["FORCE_EXIT"])
        ...
        if all(tocr["cancel_state"] is False for tocr in trade_entry_cancelation_registry):
            ...
            order_type = ordertype or self._traderpilot.strategy.order_types.get(
                "force_exit", self._traderpilot.strategy.order_types["exit"]
            )
            sub_amount: float | None = None
            if amount and amount < trade.amount:
                # Partial exit ...
                min_exit_stake = self._traderpilot.exchange.get_min_pair_stake_amount(
            ...
            self._traderpilot.execute_trade_exit(
    ...
    def _force_entry_validations(self, pair: str, order_side: SignalDirection):
        if not self._traderpilot.config.get("force_entry_enable", False):
        ...
        stake_currency = self._traderpilot.config.get("stake_currency")
        if not self._traderpilot.exchange.get_pair_quote_currency(pair) == stake_currency:
            raise RPCException(
    ...
    def _rpc_force_entry(
        ...
        *,
        ...
        if trade:
            ...
            if not self._traderpilot.strategy.position_adjustment_enable:
        ...
        if not stake_amount:
            # gen stake amount
            stake_amount = self._traderpilot.wallets.get_trade_stake_amount(
                pair, self._config["max_open_trades"]
            )

        # execute buy
        if not order_type:
            order_type = self._traderpilot.strategy.order_types.get(
                "force_entry", self._traderpilot.strategy.order_types["entry"]
            )
        with self._traderpilot._exit_lock:
            if self._traderpilot.execute_entry(
                pair,
                stake_amount,
                price,
    ...
    def _rpc_cancel_open_order(self, trade_id: int):
        ...
        with self._traderpilot._exit_lock:
            ...
            for open_order in trade.open_orders:
                try:
                    order = self._traderpilot.exchange.fetch_order(open_order.order_id, trade.pair)
                except ExchangeError as e:
                    logger.info(f"Cannot query order for {trade} due to {e}.", exc_info=True)
                    raise RPCException("Order not found.")
                self._traderpilot.handle_cancel_order(
    ...
    def _rpc_delete(self, trade_id: int) -> dict[str, str | int]:
        ...
        with self._traderpilot._exit_lock:
            ...
            for open_order in trade.open_orders:
                try:
                    self._traderpilot.exchange.cancel_order(open_order.order_id, trade.pair)
                    c_count += 1
            ...
            if (
                self._traderpilot.strategy.order_types.get("stoploss_on_exchange")
                and trade.has_open_sl_orders
            ):
                for oslo in trade.open_sl_orders:
                    try:
                        self._traderpilot.exchange.cancel_stoploss_order(oslo.order_id, trade.pair)
                        c_count += 1
                    except ExchangeError:
                        pass
    ...
    def _rpc_list_custom_data(self, trade_id: int, key: str | None) -> list[dict[str, Any]]:
        ...
        return [
            {
                "id": data_entry.id,
                "tp_trade_id": data_entry.tp_trade_id,
    ...
    def _rpc_count(self) -> dict[str, float]:
        ...
        return {
            "current": len(trades),
            "max": (
                int(self._traderpilot.config["max_open_trades"])
                if self._traderpilot.config["max_open_trades"] != float("inf")
                else -1
            ),
    ...
    def _rpc_whitelist(self) -> dict:
        ...
        res = {
            "method": self._traderpilot.pairlists.name_list,
            "length": len(self._traderpilot.active_pair_whitelist),
            "whitelist": self._traderpilot.active_pair_whitelist,
        }
        return res
    ...
    def _rpc_blacklist_delete(self, delete: list[str]) -> dict:
        ...
        errors = {}
        for pair in delete:
            if pair in self._traderpilot.pairlists.blacklist:
                self._traderpilot.pairlists.blacklist.remove(pair)
            else:
    ...
    def _rpc_blacklist(self, add: list[str] | None = None) -> dict:
        ...
        res = {
            "method": self._traderpilot.pairlists.name_list,
            "length": len(self._traderpilot.pairlists.blacklist),
            "blacklist": self._traderpilot.pairlists.blacklist,
            "blacklist_expanded": self._traderpilot.pairlists.expanded_blacklist,
            "errors": errors,
        }
        return res
    ...
    def _rpc_get_logs(limit: int | None) -> dict[str, Any]:
        ...
        #       "traderpilot.worker", "INFO", "Starting worker develop"]

        return {"log_count": len(records), "logs": records}

    def _rpc_edge(self) -> list[dict[str, Any]]:
        """Returns information related to Edge"""
        if not self._traderpilot.edge:
            raise RPCException("Edge is not enabled.")
        return self._traderpilot.edge.accepted_pairs()
    ...
    def _ws_all_analysed_dataframes(
        ...
        """
        timeframe = self._traderpilot.config["timeframe"]
        candle_type = self._traderpilot.config.get("candle_type_def", CandleType.SPOT)
    ...
    def _ws_request_analyzed_df(self, limit: int | None = None, pair: str | None = None):
        ...
        pairlist = [pair] if pair else self._traderpilot.active_pair_whitelist

        return self._ws_all_analysed_dataframes(pairlist, limit)

    def _ws_request_whitelist(self):
        """Whitelist data for WebSocket"""
        return self._traderpilot.active_pair_whitelist
    ...
    def _rpc_analysed_history_full(
        ...
        from traderpilot.data.converter import trim_dataframe
    ...
    def _rpc_plot_config(self) -> dict[str, Any]:
        if (
            self._traderpilot.strategy.plot_config
            and "subplots" not in self._traderpilot.strategy.plot_config
        ):
            self._traderpilot.strategy.plot_config["subplots"] = {}
        return self._traderpilot.strategy.plot_config
    ...
    def _rpc_plot_config_with_strategy(config: Config) -> dict[str, Any]:
        ...
        strategy.tp_load_hyper_params(False)
    ...
    def health(self) -> dict[str, str | int | None]:
        last_p = self._traderpilot.last_process
    ...
    def _update_market_direction(self, direction: MarketDirection) -> None:
        self._traderpilot.strategy.market_direction = direction

    def _get_market_direction(self) -> MarketDirection:
        return self._traderpilot.strategy.market_direction
