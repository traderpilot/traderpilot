...
from traderpilot.enums import CandleType, State, TradingMode
...
from traderpilot.rpc.api_server.deps import get_config, get_exchange, get_rpc, get_rpc_optional
...
# 2.22: Add TraderAI to backtesting
...
def show_config(rpc: RPC | None = Depends(get_rpc_optional), config=Depends(get_config)):
    ...
    if rpc:
        state = rpc._traderpilot.state
        strategy_version = rpc._traderpilot.strategy.version()
    ...
    config.update(
        {
            ...
            "strategy": strategy,
            "timerange": timerange,
            "traderaimodel": traderaimodel if traderaimodel else config.get("traderaimodel"),
        }
    )
    try:
    ...
    config.update(
        {
            ...
            "traderaimodel": (
                payload.traderaimodel if payload.traderaimodel else config.get("traderaimodel")
            ),
        }
    )
    try:
...
def list_exchanges(config=Depends(get_config)):
    from traderpilot.exchange import list_available_exchanges
...
@router.get("/traderaimodels", response_model=TraderAIModelListResponse, tags=["traderai"])
def list_traderaimodels(config=Depends(get_config)):
    from traderpilot.resolvers.traderaimodel_resolver import TraderaiModelResolver

    models = TraderaiModelResolver.search_all_objects(config, False)
    models = sorted(models, key=lambda x: x["name"])

    return {"traderaimodels": [x["name"] for x in models]}
