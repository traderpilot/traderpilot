...
from traderpilot.constants import CUSTOM_TAG_MAX_LENGTH, Config, IntOrInf, ListPairsWithTimeframes
...
from traderpilot.exceptions import OperationalException, StrategyError
from traderpilot.exchange import timeframe_to_minutes, timeframe_to_next_date, timeframe_to_seconds
from traderpilot.misc import remove_entry_exit_signals
from traderpilot.persistence import Order, PairLocks, Trade
from traderpilot.strategy.hyper import HyperStrategyMixin
from traderpilot.strategy.informative_decorator import (
    InformativeData,
    PopulateIndicators,
...
class IStrategy(ABC, HyperStrategyMixin):
    """
    Interface for traderpilot strategies
    ...
    def __init__(self, config: Config) -> None:
        ...
        self._tp_informative: list[tuple[InformativeData, PopulateIndicators]] = []
        for attr_name in dir(self.__class__):
            cls_method = getattr(self.__class__, attr_name)
            if not callable(cls_method):
                continue
            informative_data_list = getattr(cls_method, "_tp_informative", None)
            ...
            for informative_data in informative_data_list:
                ...
                self._tp_informative.append((informative_data, cls_method))
    ...
    def load_traderAI_model(self) -> None:
        if self.config.get("traderai", {}).get("enabled", False):
            ...
            # Gracious failures if traderAI is disabled but "start" is called.
            class DummyClass:
                def start(self, *args, **kwargs):
                    raise OperationalException(
                        "traderAI is not enabled. "
            ...
            self.traderai = DummyClass()  # type: ignore

    def tp_bot_start(self, **kwargs) -> None:
        """
        Strategy init - runs after dataprovider has been added.
        Must call bot_start()
        """
        self.load_traderAI_model()

        strategy_safe_wrapper(self.bot_start)()

        self.tp_load_hyper_params(self.config.get("runmode") == RunMode.HYPEROPT)

    def tp_bot_cleanup(self) -> None:
        """
        Clean up TraderAI and child threads
        """
        self.traderai.shutdown()
    ...
    def confirm_trade_entry(
        ...
        **kwargs,
        ...
        For full documentation please go to https://www.traderpilot.io/en/latest/strategy-advanced/
    ...
    def adjust_entry_price(
        ...
        **kwargs,
        ...
        For full documentation please go to https://www.traderpilot.io/en/latest/strategy-callbacks/
    ...
    def populate_any_indicators(
        ...
        This method is *only* used in TraderaiDataKitchen class and therefore
        it is only called if TraderAI is active.
    ...
    def feature_engineering_expand_all(
        ...
        *Only functional with TraderAI enabled strategies*
        ...
        https://www.traderpilot.io/en/latest/traderai-parameter-table/#feature-parameters

        https://www.traderpilot.io/en/latest/traderai-feature-engineering/#defining-the-features
    ...
    _tp_stop_uses_after_fill = False
    ...
    def __informative_pairs_traderai(self) -> ListPairsWithTimeframes:
        """
        Create informative-pairs needed for TraderAI
        """
        if self.config.get("traderai", {}).get("enabled", False):
            whitelist_pairs = self.dp.current_whitelist()
            candle_type = self.config.get("candle_type_def", CandleType.SPOT)
            corr_pairs = self.config["traderai"]["feature_parameters"]["include_corr_pairlist"]
            informative_pairs = []
            for tf in self.config["traderai"]["feature_parameters"]["include_timeframes"]:
    ...
    def gather_informative_pairs(self) -> ListPairsWithTimeframes:
        ...
        for inf_data, _ in self._tp_informative:
        ...
        informative_pairs.extend(self.__informative_pairs_traderai())
    ...
    def unlock_pair(self, pair: str) -> None:
        ...
        Not used by traderpilot itself, but intended to be used if users lock pairs
    ...
    def should_exit(
        ...
        *,
        ...
        stoplossflag = self.tp_stoploss_reached(
            current_rate=current_rate,
            trade=trade,
            current_time=current_time,
            current_profit=current_profit,
    ...
    def tp_stoploss_adjust(
        ...
        """
        if after_fill and not self._tp_stop_uses_after_fill:
    ...
    def tp_check_timed_out(self, trade: Trade, order: Order, current_time: datetime) -> bool:
        """
        FT Internal method.
        Check if timeout is active, and if the order is still open and timed out
        """
        side = "entry" if order.tp_order_side == trade.entry_side else "exit"
        ...
        time_method = (
            ...
            if order.tp_order_side == trade.exit_side
    ...
    def tp_advise_signals(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """
    ...
    def advise_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for inf_data, populate_fn in self._tp_informative:
