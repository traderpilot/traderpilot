...
from traderpilot.configuration.config_schema import (
    CONF_SCHEMA,
...
from traderpilot.configuration.deprecated_settings import process_deprecated_setting
from traderpilot.constants import UNLIMITED_STAKE_AMOUNT
from traderpilot.enums import RunMode, TradingMode
from traderpilot.exceptions import ConfigurationError
...
def _extend_validator(validator_class):
    """
    Extended validator for the Traderpilot configuration JSON Schema.
...
TraderpilotValidator = _extend_validator(Draft4Validator)
...
def validate_config_schema(conf: dict[str, Any], preliminary: bool = False) -> dict[str, Any]:
    ...
    try:
        TraderpilotValidator(conf_schema).validate(conf)
        return conf
...
def validate_config_consistency(conf: dict[str, Any], *, preliminary: bool = False) -> None:
    ...
    _validate_traderai_hyperopt(conf)
    _validate_traderai_backtest(conf)
    _validate_traderai_include_timeframes(conf, preliminary=preliminary)
...
def _validate_traderai_hyperopt(conf: dict[str, Any]) -> None:
    traderai_enabled = conf.get("traderai", {}).get("enabled", False)
    analyze_per_epoch = conf.get("analyze_per_epoch", False)
    if analyze_per_epoch and traderai_enabled:
        raise ConfigurationError(
            "Using analyze-per-epoch parameter is not supported with a TraderAI strategy."
        )


def _validate_traderai_include_timeframes(conf: dict[str, Any], preliminary: bool) -> None:
    traderai_enabled = conf.get("traderai", {}).get("enabled", False)
    if traderai_enabled:
        main_tf = conf.get("timeframe", "5m")
        traderai_include_timeframes = (
            conf.get("traderai", {}).get("feature_parameters", {}).get("include_timeframes", [])
        )
        ...
        offending_lines = []
        for tf in traderai_include_timeframes:
            tf_s = timeframe_to_seconds(tf)
            if tf_s < main_tf_s:
                offending_lines.append(tf)
        if offending_lines:
            raise ConfigurationError(
                f"Main timeframe of {main_tf} must be smaller or equal to TraderAI "
                f"`include_timeframes`.Offending include-timeframes: {', '.join(offending_lines)}"
            )

        # Ensure that the base timeframe is included in the include_timeframes list
        if not preliminary and main_tf not in traderai_include_timeframes:
            feature_parameters = conf.get("traderai", {}).get("feature_parameters", {})
            include_timeframes = [main_tf] + traderai_include_timeframes
            conf.get("traderai", {}).get("feature_parameters", {}).update(
                {**feature_parameters, "include_timeframes": include_timeframes}
            )


def _validate_traderai_backtest(conf: dict[str, Any]) -> None:
    if conf.get("runmode", RunMode.OTHER) == RunMode.BACKTEST:
        traderai_enabled = conf.get("traderai", {}).get("enabled", False)
        timerange = conf.get("timerange")
        traderai_backtest_live_models = conf.get("traderai_backtest_live_models", False)
        if traderai_backtest_live_models and traderai_enabled and timerange:
            raise ConfigurationError(
                "Using timerange parameter is not supported with "
                "--traderai-backtest-live-models parameter."
            )

        if traderai_backtest_live_models and not traderai_enabled:
            raise ConfigurationError(
                "Using --traderai-backtest-live-models parameter is only "
                "supported with a TraderAI strategy."
            )

        if traderai_enabled and not traderai_backtest_live_models and not timerange:
            raise ConfigurationError(
                "Please pass --timerange if you intend to use TraderAI for backtesting."
            )
