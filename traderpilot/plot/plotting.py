...
from traderpilot.data.history import get_timerange, load_data
...
from traderpilot.exchange import timeframe_to_prev_date, timeframe_to_seconds
from traderpilot.misc import pair_to_filename
...
from traderpilot.util import get_dry_run_wallet
...
def generate_profit_graph(
    ...
    fig["layout"].update(title="Traderpilot Profit plot")
    fig["layout"]["yaxis1"].update(title="Price")
...
def generate_plot_filename(pair: str, timeframe: str) -> str:
    ...
    file_name = "traderpilot-plot-" + pair_s + "-" + timeframe + ".html"
...
def plot_profit(config: Config) -> None:
    ...
    store_plot_file(
        fig,
        filename="traderpilot-profit-plot.html",
        directory=config["user_data_dir"] / "plot",
