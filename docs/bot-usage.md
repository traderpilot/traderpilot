...
!!! Note
    If you've used `setup.sh`, don't forget to activate your virtual environment (`source .venv/bin/activate`) before running traderpilot commands.
...
```
usage: traderpilot trade [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH]
...
```bash
traderpilot trade -c path/far/far/away/config.json
```
...
```bash
traderpilot trade -c ./config.json
```

and specify both configuration files when running in the normal Live Trade Mode:

```bash
traderpilot trade -c ./config.json -c path/to/secrets/keys.config.json
```
...
Traderpilot allows the creation of a user-data directory using `traderpilot create-userdir --userdir someDirectory`.
...
```bash
traderpilot trade --strategy AwesomeStrategy --strategy-path /some/directory
```
...
```bash
traderpilot trade -c config.json --db-url sqlite:///tradesv3.dry_run.sqlite
```
