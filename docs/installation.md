...
The traderpilot documentation describes various ways to install traderpilot
...
Please consider using the prebuilt [docker images](docker_quickstart.md) to get started quickly while evaluating how traderpilot works.
...
The easiest way to install and run Traderpilot is to clone the bot Github repository and then run the `./setup.sh` script, if it's available for your platform.
...
!!! Note
    Python3.10 or higher and the corresponding `pip` are assumed to be available. The install-script will warn you and stop if that's not the case. `git` is also needed to clone the Traderpilot repository.  
...
!!! Note "ARM64 systems"
    If you are running an ARM64 system (like a MacOS M1 or an Oracle VM), please use [docker](docker_quickstart.md) to run traderpilot.
...
=== "RaspberryPi/Raspbian"
    ...
    This image comes with python3.11 preinstalled, making it easy to get traderpilot up and running.
    ...
    git clone https://github.com/traderpilot/traderpilot.git
    cd traderpilot
...
## Traderpilot repository

Traderpilot is an open source crypto-currency trading bot, whose code is hosted on `github.com`

```bash
# Download `develop` branch of traderpilot repository
git clone https://github.com/traderpilot/traderpilot.git
...
??? Note "Install from pypi"
    An alternative way to install Traderpilot is from [pypi](https://pypi.org/project/traderpilot/). The downside is that this method requires ta-lib to be correctly installed beforehand, and is therefore currently not the recommended way to install Traderpilot.

    ``` bash
    pip install traderpilot
    ```
...
First of the ways to install Traderpilot, is to use provided the Linux/MacOS `./setup.sh` script, which install all dependencies and help you configure the bot.

Make sure you fulfill the [Requirements](#requirements) and have downloaded the [Traderpilot repository](#traderpilot-repository).

### Use /setup.sh -install (Linux/MacOS)

If you are on Debian, Ubuntu or MacOS, traderpilot provides the script to install traderpilot.

```bash
# --install, Install traderpilot from scratch
./setup.sh -i
```
...
You will run traderpilot in separated `virtual environment`

```bash
# create virtualenv in directory /traderpilot/.venv
python3 -m venv .venv
...
Traderpilot can also be installed with Miniconda or Anaconda. We recommend using Miniconda as it's installation footprint is smaller. Conda will automatically prepare and manage the extensive library-dependencies of the Traderpilot program.
...
#### Traderpilot download

Download and install traderpilot.

```bash
# download traderpilot
git clone https://github.com/traderpilot/traderpilot.git

# enter downloaded directory 'traderpilot'
cd traderpilot      
```

#### Traderpilot install: Conda Environment

```bash
conda create --name traderpilot python=3.12
```
...
#### Enter/exit traderpilot environment
...
# activate traderpilot environment
conda activate traderpilot
...
You've made it this far, so you have successfully installed traderpilot.

### Initialize the configuration

```bash
# Step 1 - Initialize user folder
traderpilot create-userdir --userdir user_data

# Step 2 - Create a new configuration file
traderpilot new-config --config user_data/config.json
```
...
```bash
traderpilot trade --config user_data/config.json --strategy SampleStrategy
```
...
```bash
# if:
bash: traderpilot: command not found
