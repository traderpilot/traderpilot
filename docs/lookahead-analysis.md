...
It also supports the lookahead-analysis of traderai strategies.
...
```
usage: traderpilot lookahead-analysis [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
- In the results table, the `biased_indicators` column will falsely flag TraderAI target indicators defined in `set_traderai_targets()` as biased. These are not biased and can safely be ignored.
