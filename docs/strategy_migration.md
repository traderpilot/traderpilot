...
We have put a great effort into keeping compatibility with existing strategies, so if you just want to continue using traderpilot in __spot markets__, there should be no changes necessary for now.
...
The `populate_any_indicators()` method has been split into `feature_engineering_expand_all()`, `feature_engineering_expand_basic()`, `feature_engineering_standard()` and`set_traderai_targets()`.
...
For a full explanation of each method, please go to the corresponding [traderAI documentation page](traderai-feature-engineering.md#defining-the-features)
...
def populate_any_indicators(
        ...
        for t in self.traderai_info["feature_parameters"]["indicator_periods_candles"]:
        ...
        for n in range(self.traderai_info["feature_parameters"]["include_shifted_candles"] + 1):
            if n == 0:
                continue
...
4. Targets - Move this part to `set_traderai_targets()`.

### traderai - feature engineering expand all
...
### Traderai - feature engineering basic
...
### TraderAI - feature engineering standard
...
### TraderAI - New data Pipeline

If you have created your own custom `ITraderaiModel` with a custom `train()`/`predict()` function, *and* you still rely on `data_cleaning_train/predict()`, then you will need to migrate to the new pipeline. If your model does *not* rely on `data_cleaning_train/predict()`, then you do not need to worry about this migration. That means that this migration guide is relevant for a very small percentage of power-users. If you stumbled upon this guide by mistake, feel free to inquire in depth about your problem in the Traderpilot discord server.

The conversion involves first removing `data_cleaning_train/predict()` and replacing them with a `define_data_pipeline()` and `define_label_pipeline()` function to your `ITraderaiModel` class:

```python  linenums="1" hl_lines="11-14 47-49 55-57"
class MyCoolTraderaiModel(BaseRegressionModel):
    """
    Some cool custom ITraderaiModel you made before Traderpilot version 2023.6
    """
    def train(
        self, unfiltered_df: DataFrame, pair: str, dk: TraderaiDataKitchen, **kwargs
    ) -> Any:
