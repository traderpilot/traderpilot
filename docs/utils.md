...
Creates the directory structure to hold your files for traderpilot.
...
```
usage: traderpilot create-userdir [-h] [--userdir PATH] [--reset]
...
```
usage: traderpilot new-config [-h] [-c PATH]
...
!!! Warning
    Only vital questions are asked. Traderpilot offers a lot more configuration possibilities, which are listed in the [Configuration documentation](configuration.md#configuration-parameters)

### Create config examples

```
$ traderpilot new-config --config user_data/config_binance.json
...
```
usage: traderpilot show-config [-h] [--userdir PATH] [-c PATH]
                             [--show-sensitive]
...
``` output
usage: traderpilot new-strategy [-h] [--userdir PATH] [-s NAME]
...
```bash
traderpilot new-strategy --userdir ~/.traderpilot/ --strategy AwesomeStrategy
```

Using the advanced template (populates all optional functions and methods)

```bash
traderpilot new-strategy --strategy AwesomeStrategy --template advanced
```
...
```
usage: traderpilot list-strategies [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                 [-d PATH] [--userdir PATH]
...
``` bash
traderpilot list-strategies
```

Example: Search strategies  directory within the userdir.

``` bash
traderpilot list-strategies --userdir ~/.traderpilot/
```

Example: Search dedicated strategy path.

``` bash
traderpilot list-strategies --strategy-path ~/.traderpilot/strategies/
```
...
```
usage: traderpilot list-hyperoptloss [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                   [-d PATH] [--userdir PATH]
...
## List traderAI models

Use the `list-traderaimodels` subcommand to see all traderAI models available.

This subcommand is useful for finding problems in your environment with loading traderAI models: modules with models that contain errors and failed to load are printed in red (LOAD FAILED), while models with duplicate names are printed in yellow (DUPLICATE NAME).

```
usage: traderpilot list-traderaimodels [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                   [-d PATH] [--userdir PATH]
                                   [--traderaimodel-path PATH] [-1] [--no-color]

optional arguments:
  -h, --help            show this help message and exit
  --traderaimodel-path PATH
                        Specify additional lookup path for traderaimodels.
  -1, --one-column      Print output in one column.
  --no-color            Disable colorization of hyperopt results. May be
                        useful if you are redirecting output to a file.
...
```
usage: traderpilot list-exchanges [-h] [-1] [-a]
...
```
$ traderpilot list-exchanges
Exchanges available for Traderpilot:
...
Example: see all exchanges supported by the ccxt library (including 'bad' ones, i.e. those that are known to not work with Traderpilot)

```
$ traderpilot list-exchanges -a
...
```
usage: traderpilot list-timeframes [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                 [-d PATH] [--userdir PATH]
                                 [--exchange EXCHANGE] [-1]
...
```
$ traderpilot list-timeframes -c config_binance.json
...
Timeframes available for the exchange `binance`: 1m, 3m, 5m, 15m, 30m, 1h, 2h, 4h, 6h, 8h, 12h, 1d, 3d, 1w, 1M
```

* Example: enumerate exchanges available for Traderpilot and print timeframes supported by each of them:
```
$ for i in `traderpilot list-exchanges -1`; do traderpilot list-timeframes --exchange $i; done
```
...
For pairs traded by Traderpilot the pair quote currency is defined by the value of the `stake_currency` configuration setting.
...
```
usage: traderpilot list-markets [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                              [-d PATH] [--userdir PATH] [--exchange EXCHANGE]
                              [--print-list] [--print-json] [-1] [--print-csv]
                              [--base BASE_CURRENCY [BASE_CURRENCY ...]]
                              [--quote QUOTE_CURRENCY [QUOTE_CURRENCY ...]]
                              [-a] [--trading-mode {spot,margin,futures}]
usage: traderpilot list-pairs [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                            [-d PATH] [--userdir PATH] [--exchange EXCHANGE]
                            [--print-list] [--print-json] [-1] [--print-csv]
                            [--base BASE_CURRENCY [BASE_CURRENCY ...]]
...
```
$ traderpilot list-pairs --quote USD --print-json
```
...
```
$ traderpilot list-pairs -c config_binance.json --all --base BTC ETH --quote USDT USD --print-list
```

* Print all markets on exchange "Kraken", in the tabular format:

```
$ traderpilot list-markets --exchange kraken --all
```
...
```
usage: traderpilot test-pairlist [-h] [--userdir PATH] [-v] [-c PATH]
                               [--quote QUOTE_CURRENCY [QUOTE_CURRENCY ...]]
...
```
traderpilot test-pairlist --config config.json --quote USDT BTC
```

## Convert database

`traderpilot convert-db` can be used to convert your database from one system to another (sqlite -> postgres, postgres -> other postgres), migrating all trades, orders and Pairlocks.
...
```
usage: traderpilot convert-db [-h] [--db-url PATH] [--db-url-from PATH]
...
!!! Warning
    Please ensure to only use this on an empty target database. Traderpilot will perform a regular migration, but may fail if entries already existed.
...
Run traderpilot in webserver mode.
Traderpilot will start the webserver and allow TraderUI to start and control backtesting processes.
...
TraderUI will also show the backtesting results.

```
usage: traderpilot webserver [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH]
                           [--userdir PATH]
...
You can use `docker compose run --rm -p 127.0.0.1:8080:8080 traderpilot webserver` to start a one-off container that'll be removed once you stop it. This assumes that port 8080 is still available and no other bot is running on that port.
...
```
usage: traderpilot backtesting-show [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                  [-d PATH] [--userdir PATH]
...
```
usage: traderpilot backtesting-analysis [-h] [-v] [--logfile FILE] [-V]
...
```
usage: traderpilot hyperopt-list [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
```
traderpilot hyperopt-list --profitable --no-details
```
...
```
usage: traderpilot hyperopt-show [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                               [-d PATH] [--userdir PATH] [--best]
...
```
traderpilot hyperopt-show -n 168
```

Prints JSON data with details for the last best epoch (i.e., the best of all epochs):

```
traderpilot hyperopt-show --best -n -1 --print-json --no-header
```
...
```
usage: traderpilot show-trades [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
``` bash
traderpilot show-trades --db-url sqlite:///tradesv3.sqlite --trade-ids 2 3 --print-json
```
...
```
usage: traderpilot strategy-updater [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                  [-d PATH] [--userdir PATH]
