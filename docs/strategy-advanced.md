...
If you're just getting started, please familiarize yourself with the [Traderpilot basics](bot-basics.md) and methods described in [Strategy Customization](strategy-customization.md) first.
...
!!! Tip
    Start off with a strategy template containing all available callback methods by running `traderpilot new-strategy --strategy MyAwesomeStrategy --template advanced`

## Storing information (Persistent)

Traderpilot allows storing/retrieving user custom information associated with a specific trade in the database.
...
For the data to be able to be stored within the database, traderpilot must serialized the data. This is done by converting the data to a JSON formatted string.
Traderpilot will attempt to reverse this action on retrieval, so from a strategy perspective, this should not be relevant.
...
``` python
from traderpilot.exchange import timeframe_to_prev_date
...
!!! Warning
    ...
    These results are a consequence of the strategy overwriting prior tags - where the last tag will "stick" and will be the one traderpilot will use.
...
!!! Note
    You should make sure to implement proper version control (like a git repository) alongside this, as traderpilot will not keep historic versions of your strategy, so it's up to the user to be able to eventually roll back to a prior version of the strategy.
...
Traderpilot provides you with an easy way to embed the strategy into your configuration file.
...
Traderpilot does however also counter this by running `dataframe.copy()` on the dataframe right after the `populate_indicators()` method - so performance implications of this should be low to non-existent.
