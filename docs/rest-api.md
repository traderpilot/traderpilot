...
TraderUI now has it's own dedicated [documentation section](trader-ui.md) - please refer to that section for all information regarding the TraderUI.
...
We advise consuming the API by using the supported `traderpilot-client` package (also available as `scripts/rest_client.py`).

This command can be installed independent of any running traderpilot bot by using `pip install traderpilot-client`.

This module is designed to be lightweight, and only depends on the `requests` and `python-rapidjson` modules, skipping all heavy dependencies traderpilot otherwise needs.

``` bash
traderpilot-client <command> [optional parameters]
```
...
``` bash
traderpilot-client --config rest_config.json <command> [optional parameters]
```

Commands with many arguments may require keyword arguments (for clarity) - which can be provided as follows:

``` bash
traderpilot-client --config rest_config.json forceenter BTC/USDT long enter_tag=GutFeeling
```

This method will work for all arguments - check the "show" command for a list of available parameters.

??? Note "Programmatic use"
    The `traderpilot-client` package (installable independent of traderpilot) can be used in your own scripts to interact with the traderpilot API.
    to do so, please use the following:

    ``` python
    from traderpilot_client import FtRestClient
...
The API Server includes a websocket endpoint for subscribing to RPC messages from the traderpilot Bot.
...
This is also used to setup [Producer/Consumer mode](producer-consumer.md) in Traderpilot.
...
For a list of message types, please refer to the RPCMessageType enum in `traderpilot/enums/rpcmessagetype.py`
...
Please make sure to replace `<traderpilot_listen_ip>` (and the subsequent port) with the IP and Port matching your configuration/setup.
...
http {
    ...
    server {
        ...
        location / {
            ...
            proxy_pass http://<traderpilot_listen_ip>:8080;
...
!!! Tip "SSL certificates"
    ...
    While this will protect your data in transit, we do not recommend to run the traderpilot API outside of your private network (VPN, SSH tunnel).
...
!!! Note
    The below should be done in an application (a Traderpilot REST API client, which fetches info via API), and is not intended to be used on a regular basis.

Traderpilot's REST API also offers JWT (JSON Web Tokens).
You can login using the following command, and subsequently use the resulting access_token.

``` bash
> curl -X POST --user Traderpilotr http://localhost:8080/api/v1/token/login
